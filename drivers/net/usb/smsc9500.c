...
#include <asm/io.h>
#include <linux/mm.h>
#if defined(CONFIG_USB_NOTIFY_LAYER) && defined(CONFIG_USE_EXT_SMSC9500)
...
unsigned int smsc_debug_mode = DBG_WARNING | DBG_INIT | DBG_LINK_CHANGE | DBG_PWR;
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,0))
	module_param(smsc_debug_mode, uint, 0);
#else
	MODULE_PARM(smsc_debug_mode,"i");
#endif
MODULE_PARM_DESC(smsc_debug_mode,"bit 0 enables trace points, bit 1 enables warning points, bit 2 enables eth gpios, bit 3 enables gen gpios");
...
static int smsc9500_private_ioctl(PADAPTER_DATA  privateData, struct usbnet *dev, PSMSC9500_IOCTL_DATA ioctlData)
{
	...
	case COMMAND_GET_CONFIGURATION:
		...
		ioctlData->Data[4]=smsc_debug_mode;
...
PHY_ACCESS_FAILURE:
	...
	case COMMAND_SET_DEBUG_MODE:
		smsc_debug_mode=ioctlData->Data[0];
		if(smsc_debug_mode&0x04UL) {
