...
struct sm5703_vib *sm_vib;

void sm5703_set_vibrator(struct regulator *reg_vib, int on)
{
	int ret = 0;
...
static void sm5703_vibrator_update(struct work_struct *work)
{
	...
	sm5703_set_vibrator(vib->vib_power, vib->state);
}
...
static int sm5703_vibrator_suspend(struct device *dev)
{
	...
	/* turn-off vibrator */
	sm5703_set_vibrator(vib->vib_power, 0);
