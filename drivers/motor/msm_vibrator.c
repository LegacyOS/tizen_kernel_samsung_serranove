...
static int msm_vibrator_probe(struct platform_device *pdev)
{
	...
	key_pinctrl = devm_pinctrl_get_select(dev, "tlmm_motor_en_active");
...
#if 0 
	rc = gpio_tlmm_config(GPIO_CFG((vib->motor_en - 902), 0, GPIO_CFG_OUTPUT,
		GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
	if (rc < 0) {
		pr_err("%s: gpio_tlmm_config is failed %d\n",__func__, rc);
