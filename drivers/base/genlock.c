...
#ifdef CONFIG_GENLOCK_TIZEN
	struct list_head waiter;  /* List of handles waiting lock */
	pid_t owner;              /* Last owner's tgid for debug */
#endif
};
...
#ifdef CONFIG_GENLOCK_TIZEN
static bool _genlock_check_condition(struct genlock *lock, int op,
					unsigned long *irqflags,
...
{
	unsigned long irqflags, ticks = msecs_to_jiffies(timeout);
	...
	/*
	 * In original code, here is 'fast path' routine when the lock state
	 * is unlocked, but if there is a race between the new lock trier and
	 * the waiter, then the later new trier could grab it instead.
	 * So remove that routine.
	 */
	...
	if (handle_has_lock(lock, handle)) {
		...
		if (flags & GENLOCK_WRITE_TO_READ) {
			if (lock->state == _WRLOCK && op == _RDLOCK) {
				...
				GENLOCK_LOG_ERR(
				"Invalid state to convert write to read\n");
				ret = -EINVAL;
				goto out;
			}
		}
		...
		if (flags & GENLOCK_WRITE_TO_READ) {
			GENLOCK_LOG_ERR(
			"Handle must have lock to convert write to read\n");
			ret = -EINVAL;
			goto out;
		}
	}
	...
	/*
	 * Adds current context wait_node to lock->waiter to distinguish the
	 * lock sequence.
	 */
	list_add_tail(&wait_node, &lock->waiter);

	spin_unlock_irqrestore(&lock->lock, irqflags);

	/*
	 * Escapes waiting routine only this turn is current context wait node.
	 * At that time the state is unlocked or
	 * is read locked when requested to read lock.
	 */
	elapsed = wait_event_interruptible_timeout(lock->queue,
				_genlock_check_condition(lock, op, &irqflags,
								&wait_node),
				ticks);
	...
	if (elapsed <= 0) {
		...
		GENLOCK_LOG_ERR("Err[%d] : locker[%d]<%s> requester[%d]<%s>\n",
				ret, lock->owner,
				lock->state == _RDLOCK ? "RD" :
					lock->state == _WRLOCK ? "WR" : "UN",
				owner,
				op == _RDLOCK ? "RD" :
					op == _WRLOCK ? "WR" : "UN");
		goto out;
