...
{
	char str[100];
	sprintf(str, "%s %s %d->%d usecases=%d\n", __FUNCTION__, client->pdata->name
	    , client->pdata->usecase->vectors->src, client->pdata->usecase->vectors->dst
	    , client->pdata->num_usecases);
	kernel_mini_tracer(str, TIME_ON);
 }
#endif
