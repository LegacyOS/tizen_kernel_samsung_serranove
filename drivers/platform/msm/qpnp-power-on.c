...
qpnp_pon_input_dispatch(struct qpnp_pon *pon, u32 pon_type)
{
	...
	pon->key_state = key_status;
	cfg->old_state = !!key_status;
	if (cfg->pon_type == PON_RESIN )
		printk(KERN_INFO "%s: volume_up [%s]\n",
				__func__, (pon_rt_sts & pon_rt_bit) ? "P" : "R");
	else {
		dev_info(&pon->spmi->dev, "%s: power_key [%s]\n", __func__,\
			(pon_rt_sts & pon_rt_bit) ? "P" : "R");
	}

	wake_lock_timeout
		(&pon->key_wake_lock, WAKELOCK_TIME);
		...
		struct device_attribute *attr, char *buf)
{
	...
	dev_info(dev, "%s: key_state= %d\n",
		__func__, pon->key_state);
...
static DEVICE_ATTR(sec_powerkey_pressed, 0444 , sysfs_key_onoff_show, NULL);

#define S2_RESET_TYPE_WARM                    0x1
#define S2_RESET_TYPE_HARD                    0x7

struct qpnp_pon_req_reg {
	/* addr */
	u16 s1_timer_addr;
	u16 s2_timer_addr;
	u16 s2_cntl_addr;
...
static int qpnp_ctrl_req(struct qpnp_pon *pon, struct qpnp_pon_req_reg *req_reg, int enable)
{
	int rc;

	if (enable) {
		/* configure s2 timer to 7 sec */
		rc = qpnp_pon_masked_write(pon, req_reg->s1_timer_addr,
					QPNP_PON_S1_TIMER_MASK, req_reg->s1_timer);
		if (rc) {
			dev_err(&pon->spmi->dev, "Unable to configure S1 timer\n");
			return rc;
		}

		/* configure s2 timer to 250ms */
		rc = qpnp_pon_masked_write(pon, req_reg->s2_timer_addr,
					QPNP_PON_S2_TIMER_MASK, req_reg->s2_timer);
		if (rc) {
			dev_err(&pon->spmi->dev, "Unable to configure S2 timer\n");
			return rc;
		}

		rc = qpnp_pon_masked_write(pon, req_reg->s2_cntl_addr,
					QPNP_PON_S2_CNTL_TYPE_MASK, req_reg->s2_cntl);
		if (rc) {
			dev_err(&pon->spmi->dev, "Unable to configure S2 reset type\n");
			return rc;
		}
		...
		/* enable S2 reset */
		rc = qpnp_pon_masked_write(pon, req_reg->s2_cntl2_addr,
					QPNP_PON_S2_CNTL_EN, QPNP_PON_S2_CNTL_EN);
		if (rc) {
			dev_err(&pon->spmi->dev, "Unable to configure S2 enable\n");
			return rc;
...
static ssize_t  show_wakeup_enable(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct qpnp_pon *pon = dev_get_drvdata(dev);
	int i;

	/* find volume_down key */
	for (i = 0; i < pon->num_pon_config; i++) {
		if (pon->pon_cfg[i].key_code == KEY_VOLUMEDOWN)
			break;
	}

	dev_info(&pon->spmi->dev, "%s: wake_enable: [%d]\n", __func__, pon->pon_cfg[i].wake_enable);

	return snprintf(buf, 40, "%d\n", pon->pon_cfg[i].wake_enable);
}
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	for (i = 0; i < pon->num_pon_config; i++) {
		if (pon->pon_cfg[i].key_code == KEY_VOLUMEDOWN) {
			pon->pon_cfg[i].wake_enable = enable;
			break;
		}
	}

	dev_info(&pon->spmi->dev, "%s: wake_enable: [%lu]\n", __func__, enable);

	return count;
}

static DEVICE_ATTR(wakeup_keys, 0666, show_wakeup_enable,  store_wakeup_enable);
...
{
	struct qpnp_pon *pon = dev_get_drvdata(dev);
	struct qpnp_pon_req_reg kpdpwr, kpdpwr_resin;
	unsigned long enable;
	int rc;
	...
	kpdpwr.s1_timer_addr = QPNP_PON_KPDPWR_S1_TIMER(pon->base);
	kpdpwr.s2_timer_addr = QPNP_PON_KPDPWR_S2_TIMER(pon->base);
	kpdpwr.s2_cntl_addr = QPNP_PON_KPDPWR_S2_CNTL(pon->base);
	kpdpwr.s2_cntl2_addr = QPNP_PON_KPDPWR_S2_CNTL2(pon->base);
	kpdpwr.s1_timer = 0xe;
	kpdpwr.s2_timer = 0x4;

	kpdpwr_resin.s1_timer_addr = QPNP_PON_KPDPWR_RESIN_S1_TIMER(pon->base);
	kpdpwr_resin.s2_timer_addr = QPNP_PON_KPDPWR_RESIN_S2_TIMER(pon->base);
	kpdpwr_resin.s2_cntl_addr = QPNP_PON_KPDPWR_RESIN_S2_CNTL(pon->base);
	kpdpwr_resin.s2_cntl2_addr = QPNP_PON_KPDPWR_RESIN_S2_CNTL2(pon->base);
	kpdpwr_resin.s1_timer = 0xe;
	kpdpwr_resin.s2_timer = 0x4;

	if (sec_debug_is_enabled()) {
		kpdpwr.s2_cntl = S2_RESET_TYPE_WARM;
		kpdpwr_resin.s2_cntl = S2_RESET_TYPE_WARM;
	} else {
		kpdpwr.s2_cntl = S2_RESET_TYPE_HARD;
		kpdpwr_resin.s2_cntl = S2_RESET_TYPE_HARD;
	}

	if (enable) {
		qpnp_ctrl_req(pon, &kpdpwr_resin, !enable);
		qpnp_ctrl_req(pon, &kpdpwr, enable);
	} else {
		qpnp_ctrl_req(pon, &kpdpwr, enable);
		qpnp_ctrl_req(pon, &kpdpwr_resin, !enable);
	}
...
static DEVICE_ATTR(reset_enable, S_IWUSR | S_IWGRP, NULL, store_reset_enable);
...
static int qpnp_pon_probe(struct spmi_device *spmi)
{
	...
	wake_lock_init(&pon->key_wake_lock,
	...
	if (IS_ERR(sec_powerkey))
		dev_err(&spmi->dev,
				"Failed to create device(sec_powerkey)!\n");

	rc = sysfs_create_group(&sec_powerkey->kobj, &sec_powerkey_attr_group);
	if (rc) {
		dev_err(&spmi->dev,
			"%s: Failed to create device file regarding sec_powerkey\n", __func__);
		return rc;
	}
...
static int qpnp_pon_remove(struct spmi_device *spmi)
{
	...
	wake_lock_destroy(&pon->key_wake_lock);
...
static int qpnp_pon_suspend(struct device *dev)
{
	...
	if (device_may_wakeup(dev)) {
		...
		for (i = 0; i < pon->num_pon_config; i++) {
			if (pon->pon_cfg[i].key_code == KEY_VOLUMEDOWN) {
				if (pon->pon_cfg[i].wake_enable) {
					enable_irq_wake(pon->pon_cfg[i].state_irq);
					break;
				}
			}
		}
...
static int qpnp_pon_resume(struct device *dev)
{
	...
	if (device_may_wakeup(dev)) {
		...
		for (i = 0; i < pon->num_pon_config; i++) {
			if (pon->pon_cfg[i].key_code == KEY_VOLUMEDOWN) {
				if (pon->pon_cfg[i].wake_enable) {
					disable_irq_wake(pon->pon_cfg[i].state_irq);
					break;
				}
			}
		}
...
static SIMPLE_DEV_PM_OPS(qpnp_pon_pm_ops, qpnp_pon_suspend, qpnp_pon_resume);
