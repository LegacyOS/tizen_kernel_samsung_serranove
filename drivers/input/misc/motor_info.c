		...
		char *buf)
{
	...
	return snprintf(buf, 20, "motor name: %s\n", mot_name);
}

static DEVICE_ATTR(motor_name, S_IRUGO, show_motor_name, NULL);
...
static int motor_info_uevent(struct device *dev, struct kobj_uevent_env *env)
{
	...
	if (ret) {
		pr_info("motor_info: add_uevent error\n");
	}
...
static void save_motor_data(void)
{
	...
	mot_data->motor_name = "NULL";
	mot_data->motor_type = "LRA";
}
...
static int create_motor_class(void)
{
	...
	if (IS_ERR(sec_motor_info)) {
		pr_info("motor_info: Failed to create class(sec_motor_info class)!!\n");
		return -EPERM;
	}
	...
	if (IS_ERR(motor_info)) {
		...
		pr_info("motor_info: Failed to create device(motor_info)\n");
		return ret;
	}
