...
#if IST30XX_USE_KEY
int ist30xx_key_code[IST30XX_MAX_KEYS + 1] = { 0, KEY_PHONE, KEY_BACK, };
#endif
		...
		struct ist30xx_data *data)
{
	...
	if (gpio_is_valid(data->dt_data->touch_en_gpio)) {
		...
		ret = gpio_direction_output(data->dt_data->touch_en_gpio, 1);
		if (ret) {
...
static int ist30xx_set_input_device(struct ist30xx_data *data)
{
	...
	input_set_abs_params(data->input_dev, ABS_MT_WIDTH_MAJOR, 0, MAX_WIDTH, 0, 0);
...
#endif
	input_set_abs_params(data->input_dev, ABS_X, 0, IST30XX_MAX_X, 0, 0);
	input_set_abs_params(data->input_dev, ABS_Y, 0, IST30XX_MAX_Y, 0, 0);

	input_set_abs_params(data->input_dev, ABS_MT_TOUCH_MAJOR, 0, DEFAULT_MAX_ABS_MT_TOUCH, 0, 0);
	input_set_abs_params(data->input_dev, ABS_MT_TOUCH_MINOR, 0, DEFAULT_MAX_ABS_MT_TOUCH, 0, 0);
