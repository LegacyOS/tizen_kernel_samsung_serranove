...
#if IST30XX_USE_KEY
int ist30xx_key_code[] = { 0, KEY_PHONE, KEY_BACK };
#endif

DEFINE_MUTEX(ist30xx_mutex);

volatile bool ist30xx_irq_working;
static bool ist30xx_initialized;
...
#endif
		input_report_key(data->input_tkey, ist30xx_key_code[id], press);
...
#endif
#ifdef CONFIG_PM
static int imagis_ist30xx_tsp_suspend(struct device *dev)
{
	struct ist30xx_data *data = ts_data;

	if (data == NULL) {
		tsp_err("tsp driver not initialized!\n");
		return 0;
	}
...
static int imagis_ist30xx_tsp_resume(struct device *dev)
{
	struct ist30xx_data *data = ts_data;
...
static SIMPLE_DEV_PM_OPS(imagis_dev_pm_ops,
			 imagis_ist30xx_tsp_suspend, imagis_ist30xx_tsp_resume);
#endif
...
static int ist30xx_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
...
#if IST30XX_USE_KEY
	input_tkey->name = "sec_touchkey";
	input_tkey->id.bustype = BUS_I2C;
	input_tkey->dev.parent = &client->dev;
#endif
	...
	input_set_abs_params(input_dev, ABS_X, 0, IST30XX_MAX_X, 0, 0);
	input_set_abs_params(input_dev, ABS_Y, 0, IST30XX_MAX_Y, 0, 0);

	input_set_abs_params(input_dev, ABS_MT_TOUCH_MAJOR, 0, DEFAULT_MAX_ABS_MT_TOUCH, 0, 0);
	input_set_abs_params(input_dev, ABS_MT_TOUCH_MINOR, 0, DEFAULT_MAX_ABS_MT_TOUCH, 0, 0);
...
#if IST30XX_USE_KEY
	{
		...
		for (i = 1; i < ARRAY_SIZE(ist30xx_key_code); i++)
			set_bit(ist30xx_key_code[i], input_tkey->keybit);
	}
#endif
...
#if IST30XX_USE_KEY
	input_unregister_device(input_tkey);
#endif
	return 0;
...
static int ist30xx_remove(struct i2c_client *client)
{
	...
	ist30xx_power_off();
#if IST30XX_USE_KEY
	input_unregister_device(data->input_tkey);
#endif
