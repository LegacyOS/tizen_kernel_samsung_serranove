...
#include "zxt_bt432_ts_hive.h"
#include "zxt_bt432_fw_hive.h"
...
{
	static struct regulator *touch_regulator_3v0;
	int ret = 0;
...
static void zinitix_vdd_on(struct bt432_ts_data *info, bool onoff)
{
	int ret;
...
static bool init_touch(struct bt432_ts_data *touch_dev)
{
	...
	u16 minor_firmware_version = 0;
	u16 reg_data_version = 0;
	u16 chip_eeprom_info;
#if USE_CHECKSUM
	u16 chip_check_sum;
	u8 checksum_err;
	...
	for (cmp_retry = 0; cmp_retry < I2C_RETRY_CNT; cmp_retry++) {
		...
		do {
			if (ts_read_data(info->client, ZINITIX_DATA_VERSION_REG,
				...
				zinitix_debug_msg("touch chip reg data version = 0x%02x\n", reg_data_version);
				read_retry = 0;
			} else {
...
static bool run_reference_DND_read(void *device_data, int button0, int button1)
{
	struct bt432_ts_data *info = (struct bt432_ts_data *)device_data;
...
static bool run_reference_HFDND_read(void *device_data, int button0, int button1)
{
	struct bt432_ts_data *info = (struct bt432_ts_data *)device_data;
	int x_num = info->cap_info.y_node_num+1, y_num = info->cap_info.x_node_num;
	int buttons[2] = {button0, button1};
...
static bool run_reference_PDiff_read(void *device_data, int button0, int button1)
{
	struct bt432_ts_data *info = (struct bt432_ts_data *)device_data;
	...
	for (i = 0; i < x_num - 1; i++) {
		for (j = 0; j < y_num - 1; j++) {
			...
			pre_val = (info->ref_scale_factor == 100) ? ((s16)dnd_h_diff[i][j])\
				: ((s16)(((s32)dnd_h_diff[i][j] * info->ref_scale_factor) / 100));
			if (diff_val > pre_val) {
				result = false;
				item_result = false;
			}
			...
			struct zxt_ts_platform_data *pdata)
{
	...
	pdata->gpio_ldo_en = of_get_named_gpio(np, "vdd_en-gpio", 0);
	...
	dev_info(dev, "%s: pdata->gpio_ldo_en = %d\n", __func__, pdata->gpio_ldo_en);
		...
		const struct i2c_device_id *i2c_id)
{
	...
	touch_dev->gpio_ldo_en_num = pdata->gpio_ldo_en;
	...
	ret = gpio_request(touch_dev->gpio_ldo_en_num, "vdd_en_gpio");
	if (ret) {
