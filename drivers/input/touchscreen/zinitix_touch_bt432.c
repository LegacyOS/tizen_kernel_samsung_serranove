...
#include "zinitix_touch_bt432.h"
#if MULTI_PROTOCOL_TYPE_B
#include <linux/input/mt.h>
#endif

#include "zinitix_touch_bt432_fw.h"
u8 *m_firmware_data_200;

#define DEF_SENSITIVITY_TH	0
#define	DEF_WIDTH_TH	3
#define	DEF_WIDTH_TH2	3
u16 g_width_th = DEF_WIDTH_TH;
...
#define TSP_TYPE_COUNT  0

#ifdef USE_OPEN_CLOSE
static void  zinitix_ts_close(struct input_dev *dev);
static int  zinitix_ts_open(struct input_dev *dev);
#endif
...
#define	USE_RESET_BEFOREHAND_TOUCH	0
#define	BEFOREHAND_TOUCH_TIME	130

#define FACTORY_MODE_CHECKING 0
#define	POST_SENSITIVITY_TIME	(60*30)
#define	POST_SENSITIVITY	130
#define POST_SENSITIVITY_BTN	280
#define FIX_COOD_Y	0
...
#ifdef USE_POST_SENSITIVITY
static struct workqueue_struct *zinitix_thr_workqueue;
#endif
...
static DEVICE_ATTR(tsp_firm_version_phone, S_IRUGO, get_tsp_firm_version_phone, NULL);	/* PHONE */
static DEVICE_ATTR(tsp_firm_version_panel, S_IRUGO, get_tsp_firm_version_panel, NULL);/*PART*/
...
struct zinitix_touch_dev {
	...
	u8 post_done;
	u16 ori_sensitivity;
	u16 ori_sensitivity_btn;
	struct timer_list post_sensitivity_tmr;
	struct timer_list *p_post_sensitivity_tmr;
	struct work_struct thr_work;
#endif

#if USE_RESET_BEFOREHAND_TOUCH
	u8 beforehand_touch_check;
	struct	timer_list beforehand_touch_tmr;
	struct	timer_list *p_beforehand_touch_tmr;
#endif
...
static bool tsp_master_enabled;
static u16 fw_AutoUpdate;

#define TSP_DEVICE_NAME "sec_touchscreen"

#define ZINITIX_DRIVER_NAME	"zinitix_ts"
...
{
	struct zinitix_touch_dev *touch_dev = container_of(work,
				struct zinitix_touch_dev, work_dvfs_chg.work);
	int ret;
	mutex_lock(&touch_dev->dvfs_lock);
	ret = set_freq_limit(DVFS_TOUCH_ID, 998400);
	mutex_unlock(&touch_dev->dvfs_lock);
...
static void set_dvfs_lock(struct zinitix_touch_dev *ts, uint32_t on)
{
	u8 ret = 0;
...
static bool ts_read_coord(struct zinitix_touch_dev *touch_dev)
{
	...
	if (touch_dev->cap_info.i2s_checksum)
		if (i2c_checksum(touch_dev,
			(s16 *)(&touch_dev->touch_info), 2) == false)
			return false;
	...
	if (touch_dev->cap_info.i2s_checksum)
		if (ts_write_reg(touch_dev->client,
			...
			sizeof(struct _ts_zinitix_coord)/2))
			!= I2C_SUCCESS)
			return false;
...
static void zinitix_vdd_on(struct zinitix_touch_dev *info, bool onoff)
{
	int ret;

	zinitix_printk("called with ON= %d\n", onoff);

	if (onoff) {
		if (info->vddo_vreg) {
			ret = regulator_enable(info->vddo_vreg);
			if (ret < 0)
				zinitix_printk("enable error: %d", ret);
		}
	} else {
		if (info->vddo_vreg) {
			ret = regulator_disable(info->vddo_vreg);
			if (ret < 0)
				zinitix_printk("disable error: %d", ret);
		}
	}
	gpio_direction_output(info->gpio_ldo_en_num, onoff);
...
static bool ts_power_control(struct zinitix_touch_dev *touch_dev, u8 ctl)
{
	...
	if (ctl == POWER_OFF) {
		zinitix_vdd_on(touch_dev, 0);

		msleep(CHIP_OFF_DELAY);
	} else if (ctl == POWER_ON_SEQUENCE) {
		zinitix_vdd_on(touch_dev, 1);
...
#if USE_RESET_BEFOREHAND_TOUCH
static void ts_beforehand_touch_handler(unsigned long data)
{
	struct zinitix_touch_dev *touch_dev = (struct zinitix_touch_dev *)data;
	if (touch_dev == NULL)
		return;
	touch_dev->p_beforehand_touch_tmr = NULL;
	zinitix_printk("beforehand tmr occured.\n");
	touch_dev->beforehand_touch_check = 0;
}

static void ts_beforehand_touch_timer_start(u16 msec, struct zinitix_touch_dev *touch_dev)
{
	if (touch_dev == NULL)
		return;
	if (touch_dev->p_beforehand_touch_tmr != NULL)
		del_timer(touch_dev->p_beforehand_touch_tmr);

	init_timer(&(touch_dev->beforehand_touch_tmr));
	touch_dev->beforehand_touch_tmr.data = (unsigned long)(touch_dev);
	touch_dev->beforehand_touch_tmr.function = ts_beforehand_touch_handler;
	touch_dev->beforehand_touch_tmr.expires = jiffies + HZ*msec/1000;
	touch_dev->p_beforehand_touch_tmr = &touch_dev->beforehand_touch_tmr;
	add_timer(&touch_dev->beforehand_touch_tmr);
	touch_dev->beforehand_touch_check = 1;
	zinitix_printk("beforehand tmr start.\n");
}

static void ts_beforehand_touch_timer_stop(struct zinitix_touch_dev *touch_dev)
{
	if (touch_dev == NULL)
		return;
	if (touch_dev->p_beforehand_touch_tmr)
		del_timer(touch_dev->p_beforehand_touch_tmr);
	touch_dev->p_beforehand_touch_tmr = NULL;
	touch_dev->beforehand_touch_check = 0;
	zinitix_printk("beforehand tmr stop.\n");
}
#endif
...
#if USE_RESET_BEFOREHAND_TOUCH
		ts_beforehand_touch_timer_start(BEFOREHAND_TOUCH_TIME, touch_dev);
#endif
	/* soft calibration */
	if (ts_write_cmd(touch_dev->client,
...
#if	DEF_SENSITIVITY_TH
	if (ts_write_reg(touch_dev->client,	ZINITIX_SENSITIVITY, DEF_SENSITIVITY_TH) != I2C_SUCCESS)
		goto fail_mini_init;
#endif
	if (ts_write_reg(touch_dev->client, 0x0037, 0X3020) != I2C_SUCCESS)
	    goto fail_mini_init;
...
#if FACTORY_MODE_CHECKING  /* 2 */
	if (1 == Factory_Mode) {
		if (ts_write_reg(touch_dev->client, 0x011e, 0x8517) != I2C_SUCCESS)
			goto fail_mini_init;

		if (ts_write_reg(touch_dev->client, 0x003f, 0x2424) != I2C_SUCCESS)
			goto fail_mini_init;
	}
#endif
...
#ifdef USE_POST_SENSITIVITY
static void zinitix_touch_thr_work(struct work_struct *work)
{
	struct zinitix_touch_dev *touch_dev =
		container_of(work, struct zinitix_touch_dev, thr_work);

	g_width_th = DEF_WIDTH_TH2;
	if (ts_write_reg(touch_dev->client,
			ZINITIX_SENSITIVITY, POST_SENSITIVITY) != I2C_SUCCESS) {
		zinitix_printk("failed to write post sensitivity.\n");
		return;
	}

	if (ts_write_reg(touch_dev->client,
			ZINITIX_BUTTON_SENSITIVITY,
			POST_SENSITIVITY_BTN) != I2C_SUCCESS) {
		zinitix_printk("failed to write post btn sensitivity.\n");
	}
}

static void ts_post_sensitivity_handler(unsigned long data)
{
	struct zinitix_touch_dev *touch_dev = (struct zinitix_touch_dev *)data;
	if (touch_dev == NULL)
		return;
	touch_dev->p_post_sensitivity_tmr = NULL;
	zinitix_printk("post timer occured.\n");
	touch_dev->post_done = 1;
	queue_work(zinitix_thr_workqueue, &touch_dev->thr_work);
}

static void ts_post_sensitivity_timer_start(u16 sec,
	struct zinitix_touch_dev *touch_dev)
{
	if (touch_dev == NULL)
		return;
	if (touch_dev->p_post_sensitivity_tmr != NULL)
		del_timer(touch_dev->p_post_sensitivity_tmr);

	init_timer(&(touch_dev->post_sensitivity_tmr));
	touch_dev->post_sensitivity_tmr.data = (unsigned long)(touch_dev);
	touch_dev->post_sensitivity_tmr.function = ts_post_sensitivity_handler;
	touch_dev->post_sensitivity_tmr.expires = jiffies + HZ*sec;
	touch_dev->p_post_sensitivity_tmr = &touch_dev->post_sensitivity_tmr;
	add_timer(&touch_dev->post_sensitivity_tmr);
	touch_dev->post_done = 0;
}

static void ts_post_sensitivity_timer_stop(struct zinitix_touch_dev *touch_dev)
{
	if (touch_dev == NULL)
		return;
	if (touch_dev->p_post_sensitivity_tmr)
		del_timer(touch_dev->p_post_sensitivity_tmr);
	flush_work(&touch_dev->thr_work);
	touch_dev->p_post_sensitivity_tmr = NULL;
	zinitix_printk("post timer stop.\n");
}
#endif
	...
	u16 curVersion, u16 curMinorVersion, u16 curRegVersion, u16 curHWID)
{
	...
	if (touch_dev->cap_info.is_zmt200 == 0) {
		...
		newVersion = (u16) (m_firmware_data_200[52] | (m_firmware_data_200[53]<<8));
		newVersion = newVersion&0xff;
		newMinorVersion = (u16) (m_firmware_data_200[56] | (m_firmware_data_200[57]<<8));
		newRegVersion = (u16) (m_firmware_data_200[60] | (m_firmware_data_200[61]<<8));
		newChipCode = (u16) (m_firmware_data_200[64] | (m_firmware_data_200[65]<<8));
		newHWID = (u16) (m_firmware_data_200[0x57d2] | (m_firmware_data_200[0x57d3]<<8));
	}
	...
	if (curHWID != newHWID)
		return false;
#endif
	zinitix_printk("cur fw_AutoUpdate state = %x\n", fw_AutoUpdate);
	if (fw_AutoUpdate == 0) {
		fw_AutoUpdate = 1;
		return false;
	}
...
retry_init:
	...
	m_firmware_data_200 = m_firmware_data_200_al;
...
#ifdef USE_POST_SENSITIVITY
#if DEF_SENSITIVITY_TH
	touch_dev->ori_sensitivity = DEF_SENSITIVITY_TH;
#else
	if (ts_read_data(touch_dev->client,
		ZINITIX_SENSITIVITY,
		(u8 *)&touch_dev->ori_sensitivity, 2) < 0)
		goto fail_init;
#endif
	zinitix_printk("touch chip sensivity = %d\n",
		touch_dev->ori_sensitivity);

	if (ts_read_data(touch_dev->client,
		ZINITIX_BUTTON_SENSITIVITY,
		(u8 *)&touch_dev->ori_sensitivity_btn, 2) < 0)
		goto fail_init;
	zinitix_printk("touch chip button sensitivity = %d\n",
		touch_dev->ori_sensitivity_btn);
	touch_dev->post_done = 0;
#endif
	...
	if (ts_check_need_upgrade(touch_dev, firmware_version, minor_firmware_version,
		...
		if (touch_dev->cap_info.is_zmt200 == 1 &&
			(ts_upgrade_firmware(touch_dev, m_firmware_data_200,
...
#if USE_RESET_BEFOREHAND_TOUCH
	if (touch_dev->beforehand_touch_check == 1) {
		zinitix_printk("beforehand touch reset perform.\r\n");
		ts_power_control(touch_dev, POWER_OFF);
		ts_power_control(touch_dev, POWER_ON_SEQUENCE);
...
#ifdef USE_POST_SENSITIVITY
	if (touch_dev->post_done) {
		zinitix_debug_msg("return post sensitivity\n");
		g_width_th = DEF_WIDTH_TH;
		ts_write_reg(touch_dev->client, ZINITIX_SENSITIVITY, touch_dev->ori_sensitivity);
		ts_write_reg(touch_dev->client, ZINITIX_BUTTON_SENSITIVITY, touch_dev->ori_sensitivity_btn);
	}
	ts_post_sensitivity_timer_start(POST_SENSITIVITY_TIME, touch_dev);
#endif
	...
	if (zinitix_bit_test(touch_dev->touch_info.status, BIT_ICON_EVENT)) {
		for (i = 0; i < touch_dev->cap_info.button_num; i++) {
			if (zinitix_bit_test(touch_dev->icon_event_reg,
				...
				input_report_key(touch_dev->input_tkey,
...
#ifdef USE_POST_SENSITIVITY
	ts_post_sensitivity_timer_stop(touch_dev);
#endif
#if USE_RESET_BEFOREHAND_TOUCH
	ts_beforehand_touch_timer_stop(touch_dev);
#endif
	zinitix_debug_msg("clear all reported points\r\n");
	zinitix_clear_report_data(touch_dev);
...
static void  zinitix_ts_close(struct input_dev *dev)
{
	struct zinitix_touch_dev *touch_dev = input_get_drvdata(dev);

	zinitix_printk("zinitix_dev_close++\n");
	zinitix_suspend(&touch_dev->client->dev);
}
static int  zinitix_ts_open(struct input_dev *dev)
{
	struct zinitix_touch_dev *touch_dev = input_get_drvdata(dev);
	zinitix_printk("zinitix_dev_open ++\n");
...
static bool ts_set_touchmode(u16 value)
{
	...
	if (ts_write_reg(misc_touch_dev->client,
		ZINITIX_TOUCH_MODE, misc_touch_dev->touch_mode) != I2C_SUCCESS)
...
static int ts_upgrade_sequence(const u8 *firmware_data)
{
	...
	printk(KERN_INFO "TSP start upgrade firmware:%d\n", __LINE__);
...
static void get_fw_ver_bin(void *device_data)
{
	...
	version = (u32)((u32)(newHWID&0xff) << 16)
		| ((newVersion&0xf) << 12)
		| ((newMinorVersion&0xf) << 8)
		| (newRegVersion&0xff);
...
static void get_config_ver(void *device_data)
{
	...
	snprintf(buff, sizeof(buff), "%s", CHIP_IC_VER);
	set_cmd_result(info, buff, strnlen(buff, sizeof(buff)));
	info->cmd_state = 2;
	dev_info(&info->client->dev, "%s: %s(%d)\n", __func__,
			buff, strnlen(buff, sizeof(buff)));
...
static void get_chip_name(void *device_data)
{
	...
	snprintf(buff, sizeof(buff), "%s", info->cap_info.is_zmt200 ? "BT432" : "BT531");
	set_cmd_result(info, buff, strnlen(buff, sizeof(buff)));
	info->cmd_state = 2;
	dev_info(&info->client->dev, "%s: %s(%d)\n", __func__,
			buff, strnlen(buff, sizeof(buff)));
...
static void fw_update(void *device_data)
{
	...
	case BUILT_IN:
		ret = ts_upgrade_sequence((u8 *)m_firmware_data_200);
	...
	case UMS:
		printk(KERN_INFO "TSP start upgrade firmware UMS\n");
		old_fs = get_fs();
		set_fs(get_ds());
		...
		if (IS_ERR(fp)) {
			printk(KERN_INFO "TSP file open error\n");
			dev_err(&client->dev,
				"file %s open error:%d\n", fw_path, (s32)fp);
			info->cmd_state = 3;
			goto err_open;
...
err_out:
	return snprintf(buf, sizeof(buf), "%s", "abnormal");
}
					   ...
					   char *buf)
{
	...
	if (ret < 0) {
		return snprintf(buf, sizeof(buf), "%s", "fail");
	}
...
#endif
{
	...
	case TOUCH_IOCTL_VARIFY_UPGRADE_DATA:
		if (copy_from_user(m_firmware_data_200,
			argp, misc_touch_dev->cap_info.ic_fw_size))
			return -I2C_FAIL;

		version = (u16) (m_firmware_data_200[52] | (m_firmware_data_200[53]<<8));
	...
	case TOUCH_IOCTL_START_UPGRADE:
		return ts_upgrade_sequence((u8 *)m_firmware_data_200);
		...
		const struct i2c_device_id *i2c_id)
{
	...
	touch_dev->input_tkey = input_allocate_device();
	if (touch_dev->input_tkey == 0) {
		printk(KERN_ERR "unabled to allocate input_tkey device \r\n");
		goto err_input_allocate_device;
	}

	/* initialize zinitix touch ic */
	touch_dev->int_gpio_num = of_get_named_gpio(np, "zinitix,irq-gpio", 0);
	touch_dev->gpio_ldo_en_num = of_get_named_gpio(np, "vdd_en-gpio", 0);

	printk(KERN_INFO "[TSP]: irq:%d, touch_en:%d\n",
		 touch_dev->int_gpio_num, touch_dev->gpio_ldo_en_num);

	if (touch_dev->int_gpio_num && touch_dev->gpio_ldo_en_num) {
		/* configure touchscreen irq gpio */
		error = gpio_request(touch_dev->int_gpio_num, "zinitix_irq_gpio");
		if (error) {
			dev_err(&client->dev, "unable to request gpio [%d]\n",
						touch_dev->int_gpio_num);
		}
		error = gpio_request(touch_dev->gpio_ldo_en_num, "zinitix_en_gpio");
		if (error) {
			dev_err(&client->dev, "unable to request gpio [%d]\n",
						touch_dev->gpio_ldo_en_num);
		}
	} else {
		dev_err(&client->dev, "irq gpio not provided\n");
	}

	touch_dev->vddo_vreg = devm_regulator_get(&touch_dev->client->dev, "vddo");
	if (IS_ERR(touch_dev->vddo_vreg)) {
		touch_dev->vddo_vreg = NULL;
		zinitix_printk("info->vddo_vreg error\n");
		ret = -EPERM;
    ...
    fw_AutoUpdate = 1;      /* when in probe, need to oneshot update */
    if (ts_init_touch(touch_dev) == false) {
...
#ifdef USE_POST_SENSITIVITY
	INIT_WORK(&touch_dev->thr_work, zinitix_touch_thr_work);
	zinitix_thr_workqueue =
		create_singlethread_workqueue("zinitix_thr_workqueue");
	if (!zinitix_thr_workqueue) {
		printk(KERN_ERR "unabled to create touch threshold work queue \r\n");
		goto err_kthread_create_failed;
	}
#endif
	...
	touch_dev->input_dev->name = TSP_DEVICE_NAME;
	...
	touch_dev->input_tkey->name = "sec_touchkey";
	touch_dev->input_tkey->id.bustype = BUS_I2C;
	touch_dev->input_tkey->dev.parent = &client->dev;
	...
	set_bit(EV_KEY, touch_dev->input_tkey->evbit);
	set_bit(EV_SYN, touch_dev->input_tkey->evbit);

	for (i = 0; i < MAX_SUPPORTED_BUTTON_NUM; i++)
		set_bit(BUTTON_MAPPING_KEY[i], touch_dev->input_tkey->keybit);
	...
	input_set_abs_params(touch_dev->input_dev, ABS_MT_WIDTH_MAJOR,
		0, 255, 0, 0);
	input_set_abs_params(touch_dev->input_dev, ABS_X, 0, touch_dev->cap_info.MaxX + ABS_PT_OFFSET, 0, 0);
	input_set_abs_params(touch_dev->input_dev, ABS_Y, 0, touch_dev->cap_info.MaxY + ABS_PT_OFFSET, 0, 0);
	...
	ret = input_register_device(touch_dev->input_tkey);
	if (ret) {
		printk(KERN_ERR "unable to register %s input device\r\n",
			touch_dev->input_tkey->name);
		goto err_input_register_device;
	}
...
#ifdef USE_OPEN_CLOSE
	touch_dev->input_dev->open = zinitix_ts_open;
	touch_dev->input_dev->close = zinitix_ts_close;
#endif
...
err_request_irq:
	...
	input_unregister_device(touch_dev->input_tkey);
err_input_register_device:
	input_free_device(touch_dev->input_dev);
	input_free_device(touch_dev->input_tkey);
err_power_sequence:
...
#ifdef USE_POST_SENSITIVITY
	ts_post_sensitivity_timer_stop(touch_dev);
	destroy_workqueue(zinitix_thr_workqueue);
#endif
#if USE_RESET_BEFOREHAND_TOUCH
	ts_beforehand_touch_timer_stop(touch_dev);
#endif
...
static ssize_t get_tsp_firm_version_phone(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	return snprintf(buf, sizeof(buf), "%s", buff);
}
...
static ssize_t get_tsp_firm_version_panel(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	return snprintf(buf, sizeof(buf), "%s", buff); /* it's connected to at_sec_handler */
}
...
static int __init zinitix_touch_init(void)
{
	...
	if (poweroff_charging) {
		printk(KERN_INFO "[TSP] ignore TSP driver because power off, poweroff_charging = %d \n", poweroff_charging);
		return 0;
	}
#endif
