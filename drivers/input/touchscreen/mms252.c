...
static int  get_tsp_connect(struct mms_ts_info *info){

	struct regulator *regulator_pullup;
	int rc = 0;
        int val,val2;
	regulator_pullup = regulator_get(&info->client->dev, "8916_l5");
	if (IS_ERR(regulator_pullup))
		return PTR_ERR(regulator_pullup);

	rc = regulator_enable(regulator_pullup);
	if (rc) {
             dev_err(&info->client->dev, "Regulator 8916_l5 enable failed rc=%d\n",rc);
             return 0;
        }
        if (info->pdata->tsp_vendor1 > 0 && info->pdata->tsp_vendor2 > 0 ) {
		val = gpio_get_value(info->pdata->tsp_vendor1);
		val2 = gpio_get_value(info->pdata->tsp_vendor2);
		dev_err(&info->client->dev, "%s : tsp_venor1=%d tsp_vendor2=%d \n", __func__ ,val ,val2);
		if ( val && val2 ) {
                     info->tsp_connect = false;
                }else {
                     info->tsp_connect = true;
		}

	}

	rc = regulator_disable(regulator_pullup);
	if (rc) {
             dev_err(&info->client->dev, "Regulator 8916_l5 enable failed rc=%d\n",rc);
             return 0;
        }
				  ...
				  const struct i2c_device_id *id)
{
	...
	if (info->pinctrl) {
        ...
        ret = get_tsp_connect(info);
	if(ret < 0) {
                dev_err(&client->dev, "%s : get_tsp_connect=-1 \n", __func__);
                ret = -EIO;
		goto err_input_alloc;
	}
        if (info->tsp_connect == false) {
                dev_err(&client->dev, "%s : TSP is not Connected \n", __func__);
                return -EIO;
	}
	...
	input_dev->phys = info->phys;
	input_dev->dev.parent = &client->dev;
	set_bit(EV_KEY, info->key_dev->evbit);
	set_bit(EV_SYN,info->key_dev->evbit);
	set_bit(pdata->key1, info->key_dev->keybit);
	set_bit(KEY_BACK, info->key_dev->keybit);
	set_bit(EV_LED, info->key_dev->evbit);
	set_bit(LED_MISC, info->key_dev->ledbit);
#endif
...
*/
	...
	input_set_abs_params(info->input_dev, ABS_MT_TRACKING_ID,0, MAX_FINGERS, 0, 0);

	input_mt_init_slots(info->input_dev, MAX_FINGERS, 0);
...
#ifdef TOUCHKEY
	input_set_drvdata(info->key_dev, info);

	ret = input_register_device(info->key_dev);
	if (ret) {
		dev_err(&client->dev, "failed to register key dev (%d)\n",
			ret);
		goto err_reg_input_dev;
	}
#endif
	...
	ret = request_threaded_irq(info->irq, NULL, mms_ts_interrupt,
...
#ifdef USE_OPEN_CLOSE
	info->input_dev->open = mms_ts_input_open;
	info->input_dev->close = mms_ts_input_close;
#endif
