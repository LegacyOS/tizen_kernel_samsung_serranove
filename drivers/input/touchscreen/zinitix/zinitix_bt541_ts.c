...
#else
u32 BUTTON_MAPPING_KEY[MAX_SUPPORTED_BUTTON_NUM] = {
	KEY_DUMMY_MENU, KEY_PHONE,	/* KEY_DUMMY_HOME1,*/
...
static bool i2c_checksum(struct bt541_ts_info *touch_dev, s16 *pChecksum,
			 u16 wlength)
{
	s16 checksum_result;
	s16 checksum_cur;
...
static bool ts_read_coord(struct bt541_ts_info *info)
{
	...
	for (i = 0; i < info->cap_info.multi_fingers; i++) {
		if (zinitix_bit_test(info->touch_info.event_flag, i)) {
			...
			if (read_data
			    (info->client, BT541_POINT_STATUS_REG + 2 + (i * 4),
		...
		if (onoff) {
			if (info->vdd_en) {
				if (regulator_is_enabled(info->vdd_en)) {
					pr_err
					    ("%s: vdd_en is already enabled\n",
					     __func__);
				} else {
					ret = regulator_enable(info->vdd_en);
					if (ret) {
						pr_err
						    ("%s: failed to enable vdd_en, %d\n",
						     __func__, ret);
						return;
					}
				}
			...
			if (info->vdd_en) {
				if (regulator_is_enabled(info->vdd_en)) {
					...
					if (ret) {
						pr_err
						    ("%s: failed to disable vdd_en, %d\n",
						     __func__, ret);
						return;
					}
				} else {
					pr_err
					    ("%s: vdd_en is already disabled\n",
					     __func__);
				}
			}
		}
...
/* In case of TSP IC's firmware is broken,
	it will always be updated to HW ID 02
	firmware even though HW 01 device exists*/
	for (i = 0; i < TSP_TYPE_COUNT; i++) {
		newHWID =
		    (u16) (m_pFirmware[i][0x7528] |
	...
	zinitix_printk(KERN_INFO
		       "firmwaretype = %d Firmware HWID = %u cap_info.hw_id = %u i = %d \n",
...
/*	new_chip_code = (u16) (firmware_data[64] | (firmware_data[65]<<8));*/
	...
	zinitix_printk
	    ("cur reg data version = 0x%x, new reg data version = 0x%x\n",
...
static bool ts_hw_calibration(struct bt541_ts_info *info)
{
	...
	if (info->cap_info.ic_int_mask != 0) {
		if (write_reg(client,
			      BT541_INT_ENABLE_FLAG, info->cap_info.ic_int_mask)
		    != I2C_SUCCESS)
			return false;
	}
...
static bool init_touch(struct bt541_ts_info *info)
{
	...
	char *checkMode = NULL;
	int max_len = strlen(saved_command_line) > strlen(productionMode) ?
		strlen(saved_command_line) : strlen(productionMode);

	checkMode = strnstr(saved_command_line, productionMode, max_len);
 retry_init:
	...
	*/
	...
	read_data(client, BT541_PERIODICAL_INTERRUPT_INTERVAL, (u8 *)&reg_val,
		  2);
#if defined(TSP_VERBOSE_DEBUG)
...
static bool mini_init_touch(struct bt541_ts_info *info)
{
	...
	/* get chip firmware version */
	if (read_data(client, BT541_HW_ID, (u8 *)&info->cap_info.hw_id, 2) <
	    0) {
		dev_err(&client->dev, "Failed to read hw id\n");
		goto fail_mini_init;
	}
...
{
	struct bt541_ts_info *info = container_of(work,
						  struct bt541_ts_info,
						  work_dvfs_chg.work);
	int retval = 0;
...
static void zinitix_set_dvfs_lock(struct bt541_ts_info *info, int32_t on)
{
	int ret = 0;
...
static void clear_report_data(struct bt541_ts_info *info)
{
	...
	for (i = 0; i < info->cap_info.button_num; i++) {
		if (info->button[i] == ICON_BUTTON_DOWN) {
			...
			input_report_key(info->input_dev, BUTTON_MAPPING_KEY[i],
					 0);
			reported = true;
...
static int bt541_pinctrl_configure(struct bt541_ts_info *info, bool active)
{
	...
	if (active) {
		set_state_tsp =
		    pinctrl_lookup_state(info->pinctrl, "tsp_en_gpio_active");
		if (IS_ERR(set_state_tsp)) {
			dev_info(&client->dev,
				 "%s: cannot get pinctrl(tsp_en_gpio) active state\n",
				 __func__);
		...
		set_state_tsp =
		    pinctrl_lookup_state(info->pinctrl, "tsp_en_gpio_suspend");
		if (IS_ERR(set_state_tsp)) {
			dev_info(&client->dev,
				 "%s: cannot get pinctrl(tsp_en_gpio) sleep state\n",
				 __func__);
			return PTR_ERR(set_state_tsp);
		}
	}
...
static void fw_update(void *device_data)
{
	...
	case UMS:
		...
		if (fsize != info->cap_info.ic_fw_size) {
			dev_err(&client->dev,
				"invalid fw size!!(fsize=%lx, ic_fw_size=%x)\n",
...
static void not_support_cmd(void *device_data)
{
	...
	snprintf(finfo->cmd_buff, strlen(finfo->cmd_buff) + 1,  "%s", "NA");
	set_cmd_result(info, finfo->cmd_buff,
		       strnlen(finfo->cmd_buff, sizeof(finfo->cmd_buff)));
...
static void run_preference_read(void *device_data)
{
	...
	for (i = 0; i < info->cap_info.x_node_num; i++) {
		for (j = 0; j < info->cap_info.y_node_num; j++) {
			...
			if (raw_data->pref_data
			    [i * info->cap_info.y_node_num + j] < min
			...
			if (raw_data->pref_data
			    [i * info->cap_info.y_node_num + j] > max)
				max =
				    raw_data->pref_data[i *
							info->
							cap_info.y_node_num +
			 ...
			 *devattr, const char *buf, size_t count)
{
	...
	cur = strnchr(buf, strlen(buf), (int)delim);
	if (cur)
					 ...
					 char *buf)
{
 ...
 err_out:
	return snprintf(buf, strlen(buf) + 1, "NG");
}
			       ...
			       unsigned int cmd, unsigned long arg)
{
 ...
 fail_hw_cal:
	...
	case TOUCH_IOCTL_SET_REG:
		...
		if (copy_from_user(&value, reg_ioctl.val, 4)) {
			misc_info->work_state = NOTHING;
			up(&misc_info->work_lock);
			pr_info("[zinitix_touch] error : copy_from_user\n");
			return val;
		...
		if (write_reg(misc_info->client,
			      reg_ioctl.addr, value) != I2C_SUCCESS)
			ret = -1;

		zinitix_debug_msg("write : reg addr = 0x%x, val = 0x%x\r\n",
				  reg_ioctl.addr, value);
		misc_info->work_state = NOTHING;
		up(&misc_info->work_lock);
		return ret;
...
*/
	if (pdata->tsp_supply_type == TSP_LDO_SUPPLY) {
		...
		if (ret) {
			pr_err
			    ("[TSP]%s: unable to request zinitix_tsp_en_gpio [%d]\n",
			     ...
			     struct bt541_ts_platform_data *pdata)
{
	...
	pr_err
	    ("%s: x_resolution :%d, y_resolution :%d, page_size :%d, orientation :%d, gpio_int :%d, vcc_en :%d\n",
...
/* Get pinctrl if target uses pinctrl */
	...
	input_tkey = input_allocate_device();
	if (input_tkey == NULL) {
		printk(KERN_ERR "unabled to allocate input device\n");
		goto err_input_register_device;
	}

	info->key_dev = input_tkey;

	info->key_dev->name = "sec_touchkey";
	info->key_dev->id.bustype = BUS_I2C;
	info->key_dev->phys = info->phys;
	info->key_dev->dev.parent = &client->dev;
...
#ifdef SUPPORTED_TOUCH_KEY
	{
		...
		for (i = 0; i < ARRAY_SIZE(BUTTON_MAPPING_KEY); i++)
			set_bit(BUTTON_MAPPING_KEY[i], input_tkey->keybit);
	}
#endif
	if (pdata->orientation & TOUCH_XY_SWAP) {
	...
	if (!info->irq) {
		dev_err(&client->dev,
			"Failed to retrieve IRQ from device tree.\n");
		ret = -ENODEV;
...
#endif
#if 0				/*TODO: block this feature, We will porting from android if nesscessay in future */
#ifndef SAMSUNG_LCD_ATTACHED
	if (!get_lcd_attached()) {
