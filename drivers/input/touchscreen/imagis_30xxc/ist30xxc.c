...
static int ist30xx_suspend(struct device *dev)
{
	...
	if (data->touch_stopped) {
		tsp_info("%s: TSP already disabled.\n", __func__);
		goto out;
	}
...
const struct dev_pm_ops ist30xx_pm_ops = {
...
extern int in_poweroff_charging(void);
static int __init ist30xx_init(void)
{
	int ret = 0;

	if (in_poweroff_charging())
		tsp_info("%s: in power off charging.\n", __func__);
	else {
		tsp_info("%s: normal booting\n", __func__);
		ret = i2c_add_driver(&ist30xx_i2c_driver);
		if (ret) {
			tsp_err("%s: registration failed. ret= %d\n",
				__func__, ret);
		}
	}
