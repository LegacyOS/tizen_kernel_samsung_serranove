...
int ist30xx_power_enable(struct ist30xx_data *data, int en)
{
	int rc = 0;
	...
	if (en) {
		if(data->dt_data->tsp_vdd_name) {
			rc = regulator_set_voltage(data->dt_data->tsp_power, 1800000, 1800000);
			if (rc)
...
int ist30xx_power_on(struct ist30xx_data *data, bool download)
{
	...
	if (data->status.power != 1) {
		...
		rc = ist30xx_power_enable(data, 1);
		if (download)
			msleep(8);
		else
			msleep(60);
...
/*		msleep(5);*/
		/* VDD disable */
		rc = ist30xx_power_enable(data, 0);
		msleep(50);
