...
static void gpio_keys_gpio_report_event(struct gpio_button_data *bdata)
{
	...
	dev_info(&input->dev, "%s: %s [%s]\n",
		__func__, button->desc, state ? "P" : "R");

	if (button->code == KEY_MENU)
		wake_lock_timeout
			(&ddata->key_wake_lock, WAKELOCK_TIME);
...
#if defined (CONFIG_SEC_HIVE_PROJECT) || defined (CONFIG_SEC_TETRA_PROJECT)
	if (button->code == KEY_RESET && state)
		kernel_restart("KEY_RESET was pressed.");
#endif
}
...
gpio_keys_get_devtree_pdata(struct device *dev)
{
	...
	for_each_child_of_node(node, pp) {
		...
		if (!of_find_property(pp, "gpios", NULL)) {
			pdata->nbuttons--;
			dev_warn(dev, "%s: Found button without gpios\n", __func__);
			continue;
		}
		...
		struct device_attribute *attr, char *buf)
{
	...
	pr_info("%s: key state=%d/%d\n",  __func__, state, ddata->pdata->nbuttons);
	return snprintf(buf, 5, "%d\n", state);
}
static DEVICE_ATTR(sec_key_pressed, 0444 , sysfs_key_onoff_show, NULL);

static ssize_t  sysfs_key_show_wakeup_enable(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct gpio_keys_drvdata *ddata = dev_get_drvdata(dev);
	const struct gpio_keys_platform_data *pdata = ddata->pdata;
	struct gpio_button_data *button = &ddata->data[0];
	char buff[40]= {0, };
	int i;
	...
	for (i = 0; i < pdata->nbuttons; i++) {
		...
		if ((button->button->type == EV_KEY) &&
			...
			if (buff[0] != 0)
				strcat(buff, ", ");
			strcat(buff, button->button->desc);
		}
	}
...
static ssize_t sysfs_key_store_wakeup_enable(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	error = bitmap_parselist(buf, bits, n_events);
	if (error) {
		pr_err("%s: bitmap_parselist fail.\n", __func__);
		goto out;
	}
...
static DEVICE_ATTR(wakeup_keys, 0666, sysfs_key_show_wakeup_enable,
					sysfs_key_store_wakeup_enable);
...
static int gpio_keys_probe(struct platform_device *pdev)
{
	...
	wake_lock_init(&ddata->key_wake_lock,
			WAKE_LOCK_SUSPEND, "keys_wake_lock");
	...
	return 0;
fail4:
	device_destroy(sec_class, sec_key->devt);
 fail3:
