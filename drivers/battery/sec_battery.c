...
static void sec_bat_swelling_check(struct sec_battery_info *battery, int temperature)
{
	...
	if (battery->status == POWER_SUPPLY_STATUS_DISCHARGING) {
		...
		battery->swelling_block = false;
		if (battery->swelling_max_vbat == 4210) {
			...
			if (temperature < battery->swelling_temp_high_recovery) {
				...
				if (temperature >= battery->swelling_temp_dischg_threshold &&
					battery->voltage_now > 4235) {
						...
						if ((battery->voltage_now < 4210) &&
							(battery->swelling_max_vbat != 4210)) {
							/* change to 4.20V float voltage */
							val.intval = 4210;
							psy_do_property(battery->pdata->charger_name, set,
								POWER_SUPPLY_PROP_VOLTAGE_MAX, val);
				...
				struct sec_battery_info *battery)
{
	...
	if (battery->charging_mode == SEC_BATTERY_CHARGING_1ST)
		...
		full_check_type = battery->pdata->full_check_type_2nd;
	pr_info("%s: type[%d], curr[%d]\n",
		__func__, full_check_type, battery->current_avg);
	...
	case SEC_BATTERY_FULLCHARGED_FG_CURRENT:
		if ((battery->current_now > 0 && battery->current_now <
			...
			battery->full_check_cnt = 0;
		pr_info("%s: type[%d], curr[%d], full_cnt[%d]\n",
			__func__, full_check_type, battery->current_now, battery->full_check_cnt);
		break;
