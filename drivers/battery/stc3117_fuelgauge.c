...
static void STC311x_TestRead(struct i2c_client *client)
{
	u32 addr = 0;
	int data[0x20];
	char *str = NULL;
    ...
    /* 0x00~0x1f Register Read */
	for (addr = 0x0; addr <= 0x1f; addr++)
		data[addr] = STC31xx_ReadByte(client, addr);

	sprintf(str+strlen(str),
		"%02xh,%02xh,%02xh,%02xh,%02xh,%02xh,%02xh,%02xh,",
...
static int STC31xx_Task(struct i2c_client *client, GasGauge_DataTypeDef *GG)
{
	...
	if (GG_Ram.reg.GG_Status != GG_RUNNING)	{
		...
		/* Enable early empty compensation code to ensure power off at critical low voltage */
#if 1
...
static irqreturn_t stc311x_fuelgauge_irq_thread(int irq, void *irq_data)
{
...
#ifdef CONFIG_OF
static int stc311x_fuelgauge_parse_dt(struct device *dev,
			struct sec_fuelgauge_info *fuelgauge)
{
...
static int stc311x_fuelgauge_probe(struct i2c_client *client,
						const struct i2c_device_id *id)
{
	...
	if (client->dev.of_node) {
		...
		error = stc311x_fuelgauge_parse_dt(&client->dev, fuelgauge);
		if (error) {
			dev_err(&client->dev,
	...
	if (!fuelgauge->client)
		printk(KERN_ALERT "Clien NULL - phuong.phd@PowerDev");
	...
	fuelgauge->psy_fg.name		= "stc3117-fuelgauge";
	...
	if (fuelgauge->pdata->fg_irq > 0) {
		...
		if (fuelgauge->fg_irq > 0) {
			ret = request_threaded_irq(fuelgauge->fg_irq,
					NULL, stc311x_fuelgauge_irq_thread,
...
static int stc311x_fuelgauge_remove(
...
static int stc311x_fuelgauge_suspend(struct device *dev)
{
...
static int stc311x_fuelgauge_resume(struct device *dev)
{
	struct sec_fuelgauge_info *fuelgauge = dev_get_drvdata(dev);
...
static void stc311x_fuelgauge_shutdown(struct i2c_client *client)
{
}

static const struct i2c_device_id stc311x_fuelgauge_id[] = {
	{"stc3117-fuelgauge", 0},
	{}
};

static const struct dev_pm_ops stc311x_fuelgauge_pm_ops = {
	.suspend = stc311x_fuelgauge_suspend,
	.resume  = stc311x_fuelgauge_resume,
};

MODULE_DEVICE_TABLE(i2c, stc311x_fuelgauge_id);
static struct of_device_id stc311x_i2c_match_table[] = {
	{ .compatible = "stc3117-fuelgauge,i2c", },
	{ },
};
MODULE_DEVICE_TABLE(i2c, stc311x_i2c_match_table);

static struct i2c_driver stc311x_fuelgauge_driver = {
	.driver = {
		   .name = "stc3117-fuelgauge",
		   .owner = THIS_MODULE,
		   .of_match_table = stc311x_i2c_match_table,
#ifdef CONFIG_PM
		   .pm = &stc311x_fuelgauge_pm_ops,
#endif
	},
	.probe	= stc311x_fuelgauge_probe,
	.remove	= stc311x_fuelgauge_remove,
	.shutdown   = stc311x_fuelgauge_shutdown,
	.id_table   = stc311x_fuelgauge_id,
};

static int __init stc311x_fuelgauge_init(void)
{
	pr_info("%s \n", __func__);

	return i2c_add_driver(&stc311x_fuelgauge_driver);
}

static void __exit stc311x_fuelgauge_exit(void)
{
	i2c_del_driver(&stc311x_fuelgauge_driver);
}

module_init(stc311x_fuelgauge_init);
module_exit(stc311x_fuelgauge_exit);
