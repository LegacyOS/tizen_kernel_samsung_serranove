...
case 1:
	...
	sel = sm5703_fled_set_movie_current_sel(fled_info, 14);
	if(sel < 0){
...
static int sm5703_fled_probe(struct platform_device *pdev)
{
	...
	cdev.brightness_set = torch_sec_set;
	cdev.default_trigger = "none";
	cdev.name = "torch-sec1";
	cdev.max_brightness = 31;
	cdev.brightness_get = NULL;

	ret = led_classdev_register(&pdev->dev, &cdev);
	if (ret < 0) {
		pr_err("%s:%d register class dev failed\n", __func__, __LINE__);
	}
