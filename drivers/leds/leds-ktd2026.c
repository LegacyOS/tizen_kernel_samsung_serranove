					  ...
					  const char *buf, size_t count)
{
	...
	pr_debug("led_lowpower mode set to %i\n", led_lowpower);
					    ...
					    const char *buf, size_t count)
{
	...
	pr_debug("led brightness set to %i\n", brightness);
					 ...
					 const char *buf, size_t count)
{
	...
	pr_debug("led pattern : %d is activated(Type:%d)\n",
				       ...
				       const char *buf, size_t count)
{
	...
	pr_debug("led_blink is called, Color:0x%X Brightness:%i\n",
				 ...
				 struct device_attribute *attr, char *buf)
{
	...
	return snprintf(buf, MAX_BUFF, "%lu\n", led->delay_on_time_ms);
}
