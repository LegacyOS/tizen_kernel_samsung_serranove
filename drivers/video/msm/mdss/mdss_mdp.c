...
{
	switch (cmd) {
	case KGSL_DRM_GET_CRTC_INFO: {
		struct msm_fb_data_type *mfd;
		struct kgsl_drm_nb_event *event =
			(struct kgsl_drm_nb_event *)_data;
		...
		crtc_info.hsize = mfd->panel_info->xres;
		crtc_info.vsize = mfd->panel_info->yres;
		crtc_info.runtime_vsync = mdss_fb_vsync_runtime_active(idx);
		crtc_info.refresh = mdss_panel_get_framerate(mfd->panel_info);

		event->data = (void *)&crtc_info;

		pr_info("fb%d:res[%d %d]vbl[%d %d]\n",
			idx, crtc_info.hsize, crtc_info.vsize,
			crtc_info.refresh, crtc_info.runtime_vsync);
	}
		break;
	case KGSL_DRM_SET_CLK: {
		int enable = (int)_data;

		pr_info("clk %d\n", (int)_data);
		mdss_mdp_clk_ctrl(enable, false);
	}
		break;
	case KGSL_DRM_SET_VSYNC: {
		struct msm_fb_data_type *mfd;
		struct kgsl_drm_nb_event *event =
			(struct kgsl_drm_nb_event *)_data;
		int idx = event->crtc;
		int en = (int)event->data;
...
#ifdef CONFIG_MSM_KGSL_DRM
	mdata->nb_ctrl.notifier_call = mdss_mdp_notifier_ctrl;
