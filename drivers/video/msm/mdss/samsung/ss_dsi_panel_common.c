...
struct dsi_panel_cmds *mdss_samsung_cmds_select(struct mdss_dsi_ctrl_pdata *ctrl, enum mipi_samsung_cmd_list cmd)
{
	...
	if (IS_ERR_OR_NULL(vdd))
		return NULL;
printk("mdss_samsung_cmds_select = %d\n",cmd);
...
static int mdss_dsi_panel_oled_set_brightness(struct backlight_device *bl) {
	struct mdss_dsi_ctrl_pdata *ctrl = vdd_data.ctrl_dsi[DISPLAY_1];
	struct samsung_display_driver_data *vdd = check_valid_ctrl(ctrl);
	printk("%s set = %d, before = %d\n", __func__,bl->props.brightness,vdd_data.bl_level);

	vdd->mfd_dsi[DISPLAY_1]->bl_level = bl->props.brightness;
	mdss_samsung_brightness_dcs(ctrl, vdd->mfd_dsi[DISPLAY_1]->bl_level);
	return 0;
}
...
{	struct mdss_dsi_ctrl_pdata *ctrl = vdd_data.ctrl_dsi[DISPLAY_1];
	struct samsung_display_driver_data *vdd = check_valid_ctrl(ctrl);
	printk("%s get = %d\n", __func__,vdd->mfd_dsi[DISPLAY_1]->bl_level);
	return vdd->mfd_dsi[DISPLAY_1]->bl_level;
}
...
static int mdss_dsi_panel_get_power(struct lcd_device *ld)
{
	...
	printk("mdss_dsi_panel_get_power power[%d]\n",vdd->vdd_blank_mode[DISPLAY_1]);

	return vdd->vdd_blank_mode[DISPLAY_1];
}
static int mdss_dsi_panel_set_power(struct lcd_device *ld, int power)
{
	...
	printk("mdss_dsi_panel_set_power power[%d]\n", power);
	...
	if (vdd->vdd_blank_mode[DISPLAY_1] == power) {
		dev_err(&ld->dev, "power mode[%d]"	\
			"it is same as previous one\n", power);
		//return -EINVAL;
	}

	vdd->vdd_blank_mode[DISPLAY_1] = power;
	pdata = &vdd->ctrl_dsi[DISPLAY_1]->panel_data;
	...
	do {
		if (pdata->event_handler)
			rc = pdata->event_handler(pdata, MDSS_EVENT_UNBLANK, NULL);
		pdata = pdata->next;
	} while (rc == 0 && pdata);
	...
	do {
		if (pdata->event_handler)
			rc = pdata->event_handler(pdata, MDSS_EVENT_BLANK, NULL);
		pdata = pdata->next;
	} while (rc == 0 && pdata);
		break;
	default:
...
#if defined(CONFIG_LCD_CLASS_DEVICE)
	lcd_device = lcd_device_register("panel", NULL, data, &ld_ops);
...
#if defined(CONFIG_BACKLIGHT_CLASS_DEVICE)
	bd = backlight_device_register("oled-bd", &lcd_device->dev,
						data, &oled_backlight_ops, &props);
	if (IS_ERR(bd)) {
		rc = PTR_ERR(bd);
