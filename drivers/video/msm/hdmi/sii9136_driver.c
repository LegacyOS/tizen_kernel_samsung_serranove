...
static void TxHW_Reset (struct sii9136_platform_data *pdata);
...
bool TPI_Init(struct sii9136_platform_data *pdata)
{
	tmdsPoweredUp = FALSE;
	...
	TxHW_Reset(pdata);									// Toggle TX reset pin
...
// RETURNS      :   None
//////////////////////////////////////////////////////////////////////////////
void TPI_Poll (struct sii9136_platform_data *pdata) {
		...
		if (((InterruptStatusImage & HOT_PLUG_STATE) >> 2) != hdmiCableConnected) {
			...
			if (hdmiCableConnected == FALSE) {
				...
				if (InterruptStatusImage == NON_MASKABLE_INT) { 			// Check if NMI has occurred
					...
					TPI_Init(pdata); 											// Reset and re-initialize
					HotPlugService();
					}
#endif
