...
static void acm_unbind(struct usb_configuration *c, struct usb_function *f)
{
	...
	if id is cleared on disconneting, The increased number is allocated on
	connecting. ACM driver can't connect to host when id is over 256 */
#if !defined(CONFIG_USB_ANDROID_SAMSUNG_COMPOSITE) && !defined(CONFIG_USB_G_SLP)
...
{
#if defined(CONFIG_USB_G_SLP) && defined(CONFIG_USB_DUN_SUPPORT)
