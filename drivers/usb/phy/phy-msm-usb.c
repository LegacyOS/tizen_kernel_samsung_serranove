...
#define USE_MUIC_CHGTYPE	(CONFIG_SM5502_MUIC | CONFIG_SM5504_MUIC |\
							| CONFIG_USB_SWITCH_RT8973 | CONFIG_SM5703_MUIC)


#ifdef USE_MUIC_CHGTYPE
	#define ENABLE_PHY_POWER_CTL (true)
#else
	#define ENABLE_PHY_POWER_CTL (false)
#endif
...
static void msm_otg_start_host(struct usb_otg *otg, int on)
{
	...
	if (!otg->host)
		return;
#if defined (CONFIG_USB_HOST_NOTIFY) && defined (CONFIG_USB_HOST_PHY_TUNE)
	...
	case USB_OTG:
		if (pdata->otg_control == OTG_USER_CONTROL) {
			...
			if (pdata->pmic_id_irq) {
				...
				if (pdata->default_mode == USB_HOST) {
					dev_info(motg->phy.dev, "Set default_mode USB_HOST\n");
					clear_bit(ID, &motg->inputs);
				} else {
					//****ADD THIS BLOCK
					/* External ID notifier - default to ID SET */
					set_bit(ID, &motg->inputs);
				}
			}
#ifdef OTG_WAIT_PMIC
