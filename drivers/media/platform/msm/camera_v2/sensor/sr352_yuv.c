...
int sr352_sensor_match_id(struct msm_sensor_ctrl_t *s_ctrl)
{
    ...
    if(chipid!=slave_info->sensor_id){
        pr_err("[SR352] %s: chipid read=%x did not match with chipid=%x",
		__func__, chipid, slave_info->sensor_id);
...
#if defined (AF_FLASH_SUPPORT) && !defined(CONFIG_SEC_KIRANLTE_PROJECT) && !defined(CONFIG_SEC_GRANDPRIME_PROJECT) && \
	!defined(CONFIG_SEC_J5LTE_PROJECT) && !defined(CONFIG_SEC_MILLET_PROJECT) && \
	!defined(CONFIG_SEC_HIVE_PROJECT) && !defined(CONFIG_SEC_M2M_PROJECT) && \
	!defined(CONFIG_SEC_KITT_PROJECT) && !defined(CONFIG_SEC_TETRA_PROJECT) && \
	!defined(CONFIG_SEC_Z3LTE_CIS_PROJECT) && !defined(CONFIG_SEC_Z3LTE_CHN_PROJECT)
        ...
        if (burst_flag == 0) {
            ...
            if (subaddr == BURST_REG && value == 0x00) {
                ...
                if (seq_flag) {
                    ...
                    err = s_ctrl->sensor_i2c_client->i2c_func_tbl->\
                        i2c_write_burst_table(s_ctrl->sensor_i2c_client, &conf_array);
                    if (err < 0) {
