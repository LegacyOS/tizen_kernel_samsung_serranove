...
static int ims1911_leda_onoff(struct ims1911_data *data, bool onoff)
{
	int ret;
	...
	if (onoff)
		ret = gpio_direction_output(data->en_gpio_num, 1);
	else
		ret = gpio_direction_output(data->en_gpio_num, 0);

	if (ret < 0)
		ims_err("error for direction\n");
...
static int ims1911_vdd_control(struct ims1911_data *data, bool onoff)
{
	...
	vio = devm_regulator_get(dev, "ims1911-vio");
	if (IS_ERR(vio)) {
		ims_err("cannot get vio\n");
		return -ENOMEM;
	}
...
static int ims1911_setup_en_gpio(struct ims1911_data *data)
{
	int ret;

	ret = gpio_request(data->en_gpio_num, "gpio_proximity_en");
	if (ret < 0) {
		ims_err("gpio %d request failed (%d)\n", data->en_gpio_num, ret);
		return ret;
	}

	ret = gpio_direction_output(data->en_gpio_num, 0);
	if (ret < 0) {
		ims_err("failed gpio %d as output (%d)\n", data->en_gpio_num, ret);
		goto err_gpio_direction_output;
	}

	ims_info("request_en_gpio(%d) success\n", data->en_gpio_num);

	goto done;

err_gpio_direction_output:
	gpio_free(data->en_gpio_num);
done:
...
static int ims1911_parse_dt(struct ims1911_data *data, struct device *dev)
{
	...
	data->en_gpio_num = of_get_named_gpio_flags(np, "ims1911,en-gpio", 0,
		&flags);
	if (data->en_gpio_num < 0) {
		ims_err("get en_gpio(%d) error\n", data->en_gpio_num);
		return -ENODEV;
	}
	...
	const struct i2c_device_id *id)
{
	...
	ret = ims1911_setup_en_gpio(data);
	if (ret < 0)
