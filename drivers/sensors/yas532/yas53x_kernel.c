...
static int yas53x_i2c_open(int32_t type)
{
	pr_info("[SENSOR] %s\n", __func__);
	return 0;
}

static int yas53x_i2c_close(int32_t type)
{
	pr_info("[SENSOR] %s\n", __func__);
	return 0;
}

static int yas53x_i2c_write(int32_t type, uint8_t addr, const uint8_t *buf, int len)
{
	uint8_t tmp[16];
	if (sizeof(tmp) - 1 < len)
		return -1;
...
static int yas53x_i2c_read(int32_t type, uint8_t addr, uint8_t *buf, int len)
{
...
static void yas53x_msleep(int us)
{
	usleep_range(us, us + 1000);
}

static uint32_t yas53x_current_time(void)
{
...
static void yas53x_input_work_func(struct work_struct *work)
{
	...
	time_before= yas53x_current_time();
	rt = this_drv.measure(&mag,1);
/*
	if (rt & YAS_REPORT_HARD_OFFSET_CHANGED)
		this_drv.get_offset(data->hard_offset);
*/
	time_after = yas53x_current_time();
	...
	struct device_attribute *attr,	char *buf)
{
	...
	mutex_lock(&data->mutex);
	rt = this_drv.ext(YAS537_SELF_TEST,&r);
	mutex_unlock(&data->mutex);
	...
	struct device_attribute *attr,	char *buf)
{
	...
	int rt;
	mutex_lock(&data->mutex);
	rt = this_drv.ext(YAS537_SELF_TEST_NOISE,&raw_xyz);
	mutex_unlock(&data->mutex);
