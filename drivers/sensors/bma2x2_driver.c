...
int bma2x2_get_sleep_duration1(struct i2c_client *client, unsigned char
		*sleep_dur)
{
	int comres = 0;
	unsigned char data;
...
int bma2x2_set_sleep_duration1(struct i2c_client *client, unsigned char
		sleep_dur)
{
	int comres = 0;
	unsigned char data;
...
const int bma2x2_sensor_bitwidth1[] = {
	12,  10,  8, 14
};
...
#ifndef BMA2X2_SENSOR_IDENTIFICATION_ENABLE
	bitwidth = bma2x2_sensor_bitwidth1[sensor_type];
		...
		struct device_attribute *attr, char *buf)
{
	...
	if (bma2x2_get_sleep_duration1(bma2x2->bma2x2_client, &data) < 0)
		return -EINVAL;
		...
		const char *buf, size_t count)
{
	...
	if (error)
		return error;
	if (bma2x2_set_sleep_duration1(bma2x2->bma2x2_client,
				(unsigned char) data) < 0)
		return -EINVAL;
...
#ifndef BMA2X2_SENSOR_IDENTIFICATION_ENABLE
			acc_lsb.x >>=
			(16 - bma2x2_sensor_bitwidth1[bma2x2->sensor_type]);
			acc_lsb.y >>=
			(16 - bma2x2_sensor_bitwidth1[bma2x2->sensor_type]);
			acc_lsb.z >>=
			(16 - bma2x2_sensor_bitwidth1[bma2x2->sensor_type]);
		...
		struct device_attribute	*attr, const char *buf,	size_t count)
{
	...
	case 0:
		...
		if (atomic_read(&bma2x2->factory_mode)) {
			bma2x2_set_bandwidth(bma2x2->bma2x2_client,
						bma2x2->cur_bw);
			usleep_range(3000, 3100);
...
static void bma2x2_high_g_interrupt_handle(struct bma2x2_data *bma2x2)
{
	...
	int i;
		...
		for (i = 0; i < 3; i++) {
			bma2x2_get_HIGH_first(bma2x2->bma2x2_client,
				i, &first_value);
			if (first_value == 1) {
		...
		pr_info("%s High G interrupt happened,exis is %d, first is %d,sign is %d\n",
...
static void bma2x2_irq_work_func(struct work_struct *work)
{
	...
	unsigned long nanosec_rem;
#endif
	pr_info("bma2x2_irq_work_func start\n");
#ifdef CONFIG_BMA_ENABLE_NEWDATA_INT
	static struct bma2x2acc accvalue;
	...
	if ((status&0x80) == 0x80) {
		...
		bma2x2_read_accel_xyz(bma2x2->bma2x2_client,
					bma2x2->sensor_type, &accvalue);
		input_report_abs(bma2x2->input, ABS_X, accvalue.x);
		input_report_abs(bma2x2->input, ABS_Y, accvalue.y);
		input_report_abs(bma2x2->input, ABS_Z, accvalue.z);
		input_sync(bma2x2->input);
		mutex_lock(&bma2x2->value_mutex);
		bma2x2->value = accvalue;
		mutex_unlock(&bma2x2->value_mutex);
		return;
	}
#endif
...
static int bma2x2_acc_power_onoff(struct bma2x2_data *data, bool onoff)
{
	...
	data->reg_vdd = devm_regulator_get(&data->bma2x2_client->dev,
					"bma2x2,vdd");
...
static int bma2x2_parse_dt(struct bma2x2_data *data, struct device *dev)
{
	...
	data->acc_int1 = of_get_named_gpio_flags(np,
				"bma2x2,gpio_int1", 0, &flags);
	if (data->acc_int1 < 0) {
		...
		const struct i2c_device_id *id)
{
	...
	*/
	...
	err = sensors_register(bma2x2_device, data, bma2x2_attributes_sensors,
