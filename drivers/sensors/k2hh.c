...
static int k2hh_regulator_onoff(struct k2hh_p *data, bool onoff)
{
	...
	data->vdd = devm_regulator_get(&data->client->dev, "stm,reg_vdd");
	if (IS_ERR(data->vdd)) {
...
#ifdef CONFIG_WAKE_UP_MOTION
static void k2hh_enable_interrupt_wake_up (struct k2hh_p *data, bool enable)
{
...
#ifdef CONFIG_WAKE_UP_MOTION
		k2hh_enable_interrupt_wake_up(data, true);
#endif
		if (pre_enable == ON) {
...
static irqreturn_t k2hh_irq_thread(int irq, void *k2hh_data_p)
{
	...
	data->irq_state = 1;
#ifdef CONFIG_WAKE_UP_MOTION
	time_diff = jiffies_to_msecs(jiffies - data->motion_recg_st_time);
	if (time_diff > 1000){
		input_event (data->input_wakeup, EV_KEY, KEY_HOMEPAGE,1);
		input_sync(data->input_wakeup);

		msleep (10);

		input_event (data->input_wakeup, EV_KEY, KEY_HOMEPAGE,0);
		input_sync(data->input_wakeup);
	}
	k2hh_set_odr(data);
	if (atomic_read(&data->enable) == OFF) {
...
#ifdef CONFIG_WAKE_UP_MOTION
	dev_wakeup = input_allocate_device();
	if (!dev_wakeup)
		return -ENOMEM;

	dev_wakeup->name = "LPM_MOTION_WAKEUP";
	dev_wakeup->id.bustype = BUS_I2C;

	input_set_capability(dev_wakeup, EV_KEY, KEY_HOMEPAGE);

	ret = input_register_device(dev_wakeup);
	if (ret < 0) {
		input_free_device(dev_wakeup);
		return ret;
	}

	data->input_wakeup = dev_wakeup;
#endif
	return 0;
}
