...
static struct adreno_device device_3d0 = {
	...
	.start_work = __WORK_INITIALIZER(device_3d0.start_work,
		adreno_start_work),
	.input_work = __DELAYED_WORK_INITIALIZER(device_3d0.input_work,
		adreno_input_work, 0),
};
...
/* Delays to wake up adreno GPU by touch events (ms) */
#define INPUT_WORK_DELAY	70
...
static void adreno_input_work(struct work_struct *work)
{
	struct adreno_device *adreno_dev = container_of(to_delayed_work(work),
		...
		unsigned int code, int value)
{
	...
	if ((type == EV_ABS) &&
		...
		schedule_delayed_work(&adreno_dev->input_work,
					msecs_to_jiffies(INPUT_WORK_DELAY));
	}
}
