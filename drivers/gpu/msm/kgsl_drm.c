...
struct drm_kgsl_private {
	...
	bool fake_vbl;
	struct work_struct disable_vbl_work;
	struct work_struct fake_vbl_work;
	struct mutex fake_vbl_lock;
	atomic_t vbl_received[DRM_KGSL_CRTC_MAX];
	unsigned long vblank_interval_us[DRM_KGSL_CRTC_MAX];
	atomic_t vbl_runtime_active[DRM_KGSL_CRTC_MAX];
	atomic_t vbl_skip[DRM_KGSL_CRTC_MAX];
	u32 dbg_cnt;
};
...
static BLOCKING_NOTIFIER_HEAD(kgsl_drm_nb_list);
...
int
kgsl_gem_obj_addr(int drm_fd, int handle, dma_addr_t *start,
			unsigned long *len)
{
	struct file *filp;
		    ...
		    struct drm_file *file_priv)
{
	...
	DRM_INFO("%s:h[%d]s[%d]f[0x%x]o[0x%x][%lld us]\n",
		"ga",args->handle, (int)obj->size, priv->type,
		(int)obj, time_end - time_start);
			   ...
			   struct drm_file *file_priv)
{
	...
	args->size = obj->size * priv->bufcount;
	ret = 0;
...
static void kgsl_drm_prepare_vblank(struct drm_device *dev, int crtc)
{
	struct drm_kgsl_private *dev_priv =
		(struct drm_kgsl_private *)dev->dev_private;
	struct kgsl_drm_nb_event event;

	if (dev_priv->dbg_cnt)
		DRM_INFO("[wait_vbl:crtc%d]\n", crtc);
	...
	switch (crtc) {
	case DRM_KGSL_CRTC_PRIMARY:
		if (cancel_work_sync(&dev_priv->disable_vbl_work))
			DRM_INFO("%s:cancel disable_vbl_work\n", __func__);
		event.crtc = crtc;
		event.data = (void *)1;
		kgsl_drm_nb_send_event(KGSL_DRM_SET_VSYNC, (void *)&event);
		break;
	default:
		break;
	}
...
static void
kgsl_drm_disable_vblank_work(struct work_struct *work)
{
	struct kgsl_drm_nb_event event;

	event.crtc = DRM_KGSL_CRTC_PRIMARY;
	event.data = (void *)0;
	kgsl_drm_nb_send_event(KGSL_DRM_SET_VSYNC, (void *)&event);
}
...
kgsl_drm_disable_vblank(struct drm_device *dev, int crtc)
{
	...
	case DRM_KGSL_CRTC_PRIMARY:
		if (cancel_work_sync(&dev_priv->disable_vbl_work))
			DRM_INFO("%s:cancel disable_vbl_work\n", __func__);
		schedule_work(&dev_priv->disable_vbl_work);
		break;
...
static void kgsl_drm_handle_vblank(struct drm_device *dev,
			enum drm_kgsl_crtc_id crtc)
{
	struct drm_kgsl_private *dev_priv =
		(struct drm_kgsl_private *)dev->dev_private;

	if (atomic_read(&dev_priv->vbl_skip[crtc]) &&
		dev_priv->vblank_interval_us[crtc] > VBL_DEV_MS) {
		atomic_set(&dev_priv->vbl_skip[crtc], 0);
		return;
	}

	if (atomic_read(&dev->vblank_refcount[crtc]))  {
		if (dev_priv->dbg_cnt) {
			DRM_INFO("[handle_vbl:crtc%d]\n", crtc);
			dev_priv->dbg_cnt--;
		}
	...
	atomic_set(&dev_priv->vbl_skip[crtc], 1);
 }
...
/* FIXME!! : we need to check below dev_priv */
#if 0
	struct drm_kgsl_private *dev_priv =
		(struct drm_kgsl_private *)dev->dev_private;
#endif
	...
	DRM_DEBUG_KMS("%s:isr[0x%x] mdp_mdrm_intr_status =%u \n", __func__, isr,
			mdp_drm_intr_status);
	...
	if (isr & MDSS_MDP_INTR_INTF_1_VSYNC) {
		DRM_DEBUG_KMS("%s:DSI0\n", __func__);
		kgsl_drm_handle_vblank(dev, DRM_KGSL_CRTC_PRIMARY);
	}

	if (isr & MDSS_MDP_INTR_WB_0_DONE) {
		DRM_DEBUG_KMS("%s:Rotator\n", __func__);
		kgsl_drm_handle_vblank(dev, DRM_KGSL_CRTC_ROTATOR);
	}

	if (isr & MDSS_MDP_INTR_WB_2_DONE) {
		DRM_DEBUG_KMS("%s:WFD\n", __func__);
		kgsl_drm_handle_vblank(dev, DRM_KGSL_CRTC_WFD);
	}
...
static int
kgsl_drm_calculate_dfps_interval(struct drm_kgsl_private *dev_priv,
			enum drm_kgsl_crtc_id crtc, int dfps)
{
	/* ToDo: need to support additional condition. */
	switch (crtc) {
	case DRM_KGSL_CRTC_PRIMARY:
		if (dfps != 30 && dfps != 60) {
			DRM_ERROR("do not support fps[%d] on crtc[%d]\n",
					dfps, crtc);
			return -EINVAL;
		}
		break;
	case DRM_KGSL_CRTC_FAKE:
		...
		break;
	default:
		DRM_ERROR("do not support crtc[%d]\n", crtc);
		return -EINVAL;
	}

	dev_priv->vblank_interval_us[crtc] = (unsigned long int)
		(VBLANK_INTERVAL(dfps));
	DRM_INFO("%s:vblank interval %lu on crtc %d\n",
			__func__, dev_priv->vblank_interval_us[crtc], crtc);

	return 0;
}

int
kgsl_drm_set_dfps_ioctl(struct drm_device *dev, void *data,
		      struct drm_file *file_priv)
{
	struct drm_kgsl_private *dev_priv =
		(struct drm_kgsl_private *)dev->dev_private;
	struct drm_kgsl_vblank_fps *drm_vbl_dfps;
	enum drm_kgsl_crtc_id crtc;
	int ret = 0, dfps;

	if (data == NULL) {
		DRM_ERROR("%s:data null\n", __func__);
		return -EINVAL;
	}

	drm_vbl_dfps = data;
	crtc = drm_vbl_dfps->crtc;
	dfps = drm_vbl_dfps->dfps;
	...
	DRM_INFO("%s:crtc[%d]dfps[%d]\n", __func__, crtc, dfps);

	ret = kgsl_drm_calculate_dfps_interval(dev_priv, crtc, dfps);
...
kgsl_drm_fake_vblank_handler(struct work_struct *work)
{
	...
	usleep_range(dev_priv->vblank_interval_us[DRM_KGSL_CRTC_FAKE],
			dev_priv->vblank_interval_us[DRM_KGSL_CRTC_FAKE]+1000);
...
kgsl_drm_irq_preinstall(struct drm_device *dev)
{
	...
	for (event.crtc = 0; event.crtc < DRM_KGSL_CRTC_MAX; event.crtc++) {
		atomic_set(&dev_priv->vbl_received[event.crtc], 0);

		if (!kgsl_drm_nb_send_event(KGSL_DRM_GET_CRTC_INFO,
				(void *)&event)){
			struct kgsl_drm_crtc_info *crtc_info =
				(struct kgsl_drm_crtc_info *)event.data;

			DRM_INFO("%s:crtc[%d]res[%d %d]vbl[%d %d]\n",
				__func__, event.crtc,
				crtc_info->hsize, crtc_info->vsize,
				crtc_info->refresh, crtc_info->runtime_vsync);

			dev_priv->vblank_interval_us[event.crtc] =
				(crtc_info->refresh == 30) ?
				VBL_DEV_MS *2 : VBL_DEV_MS;
			atomic_set(&dev_priv->vbl_runtime_active[event.crtc],
				crtc_info->runtime_vsync);
		} else {
			dev_priv->vblank_interval_us[event.crtc] = VBL_DEV_MS;
			atomic_set(&dev_priv->vbl_runtime_active[event.crtc], 0);
		}
	}
...
/* FIXME:!! we already changed clk ctrl to notifier */
#if 0
...
kgsl_drm_irq_uninstall(struct drm_device *dev)
{
	...
	/* ToDo: need to support both cmd / video panel */
	DRM_INFO("%s\n", __func__);
	...
	for (i = 0; i < DRM_KGSL_CRTC_MAX; i++) {
		...
		atomic_set(&dev_priv->vbl_runtime_active[i], 0);
	}
...
int kgsl_drm_nb_register(struct notifier_block *nb)
{
	return blocking_notifier_chain_register(
		&kgsl_drm_nb_list, nb);
}

int kgsl_drm_nb_unregister(struct notifier_block *nb)
{
	return blocking_notifier_chain_unregister(
		&kgsl_drm_nb_list, nb);
}

int kgsl_drm_nb_send_event(unsigned long val, void *v)
{
	return blocking_notifier_call_chain(
		&kgsl_drm_nb_list, val, v);
}
...
static int kgsl_drm_load(struct drm_device *dev, unsigned long flags)
{
	...
	/* init workqueue for disable vsync */
	INIT_WORK(&dev_priv->disable_vbl_work, kgsl_drm_disable_vblank_work);
	 ...
	 * 0 : primary device(LCD)
	 * 1 : secondary device(HDMI)
	 * 2 : writeback 0 device(Rotator)
	 * 3 : writeback 2 device(WFD)
	 */
...
struct drm_ioctl_desc kgsl_drm_ioctls[] = {
	...
	DRM_IOCTL_DEF_DRV(KGSL_GEM_CREATE_FD, kgsl_gem_create_fd_ioctl,
		      DRM_MASTER),
	DRM_IOCTL_DEF_DRV(KGSL_DRM_SET_DFPS,
				kgsl_drm_set_dfps_ioctl, DRM_MASTER),
};
...
static struct drm_driver driver = {
	...
	.prepare_vblank = kgsl_drm_prepare_vblank,
