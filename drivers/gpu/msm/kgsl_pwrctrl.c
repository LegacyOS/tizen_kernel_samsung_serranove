...
#if defined (CONFIG_SYSTEM_LOAD_ANALYZER)
	store_external_load_factor(GPU_FREQ, pwrlevel->gpu_freq);
#endif
...
#ifdef CONFIG_TIZEN_KGSL_PWRCTRL_SET_DEF_MIN_LVL_310
	if (level > pwr->num_pwrlevels - 3)
		level = pwr->num_pwrlevels - 3;
#endif

#ifdef CONFIG_TIZEN_KGSL_PWRCTRL_POLICY
	if (level <= pwr->max_pwrlevel) {
		if (pwr->bak_min_pwrlevel != -1) {
			pwr->min_pwrlevel = pwr->bak_min_pwrlevel;
			pwr->bak_min_pwrlevel = -1;
		}
	} else {
		if (level > pwr->min_pwrlevel) {
			if (pwr->bak_min_pwrlevel == -1)
				pwr->bak_min_pwrlevel = pwr->min_pwrlevel;
...
int kgsl_pwrctrl_wake(struct kgsl_device *device, int priority)
{
	...
	case KGSL_STATE_NAP:
		...
		pm_qos_update_request(&device->pwrctrl.pm_qos_req_dma,
				device->pwrctrl.pm_qos_latency);
#if defined (CONFIG_SYSTEM_LOAD_ANALYZER)
		store_external_load_factor(GPU_FREQ, pwr->pwrlevels[pwr->active_pwrlevel].gpu_freq);
#endif
	case KGSL_STATE_ACTIVE:
		kgsl_pwrctrl_request_state(device, KGSL_STATE_NONE);
		break;
...
{
#if defined (CONFIG_SYSTEM_LOAD_ANALYZER)
	store_external_load_factor(GPU_FREQ, 0);
