...
#define SEC_CSA_MAC_FILEPATH	"/csa/.mac.info"
#define SEC_MAC_FILEPATH	"/opt/etc/.mac.info"
...
static int wlan_hdd_read_mac_addr(unsigned char *mac)
{
	...
	char randommac[3]    = {0};
	char buf[18]         = {0};
	char *csa_filepath       = SEC_CSA_MAC_FILEPATH;
	...
	for (i = 0; i < MAX_RETRY; ++i) {
		fp = filp_open(csa_filepath, O_RDONLY, 0);
		if (IS_ERR(fp) || create_random_mac ==1) {
			/* File Doesn't Exist. Create and write mac addr.*/
			fp = filp_open(csa_filepath, O_RDWR | O_CREAT, 0660);
			if (IS_ERR(fp)) {
			...
			if (fp->f_mode & FMODE_WRITE) {
				...
				if (ret < 0)
					printk("[WIFI] MAC write to %s is failed: %s", csa_filepath, macbuffer);
				else
					printk("[WIFI] MAC write to %s is success: %s", csa_filepath, macbuffer);
			}
			set_fs(oldfs);
		}
	...
	/* Writing MAC ID to the /opt/etc/.mac.info */
	fp = filp_open(filepath, O_RDWR | O_CREAT, 0666);

	if (IS_ERR(fp)) {
		printk("[WIFI_SEC] %s: File open error\n", filepath);
		return -1;
	} else {
		oldfs = get_fs();
		set_fs(get_ds());
		...
		if (fp->f_mode & FMODE_WRITE) {
			ret = fp->f_op->write(fp, (const char *)buf,
				sizeof(buf), &fp->f_pos);
			if (ret < 0)
				printk("[WIFI_SEC] Mac address [%s] Failed"
					" to write into File: %s\n", buf, filepath);
			else
				printk("[WIFI_SEC] Mac address [%s] written"
					" into File: %s\n", buf, filepath);
		}
		set_fs(oldfs);
		filp_close(fp, NULL);
...
#define SEC_VERSION_FILEPATH	"/opt/etc/.wifiver.info"
