...
/* Increased the timeout from 500msec to 3000msec because
 * 500msec is very short duration if we consider a case of pairing request
 * upon pairing request we need to show popup even if device is in suspend
 * state with 500msec we were release in wake lock in 500msec which is not
 * sufficient time for display to come up and hold the wake lock and show
 * popup */
#define RX_Q_MONITOR		(3000)	/* 3000 milli second, 3 seconds*/
