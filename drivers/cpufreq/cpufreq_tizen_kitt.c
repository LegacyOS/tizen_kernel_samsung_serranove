/*
 *  drivers/cpufreq/cpufreq_tizen_kitt.c
 *
...
#ifndef CONFIG_CPU_FREQ_DEFAULT_GOV_TIZEN_KITT
static struct cpufreq_governor cpufreq_gov_tizen_kitt;
#endif

static unsigned int default_powersave_bias;

static void tizen_kitt_powersave_bias_init_cpu(int cpu)
{
...
static void tizen_kitt_powersave_bias_init(void)
{
	int i;
	for_each_online_cpu(i) {
		tizen_kitt_powersave_bias_init_cpu(i);
	}
}
...
int needed_cpu_online_kitt(unsigned int cpu_load[], unsigned int cpu_freq_mhz)
{
...
void __ref cpu_sim_hotplug_work_kitt(struct work_struct *work)
{
	int cpu;
	int num_online_cpu;
 ...
 */
static void tk_check_cpu(int cpu, unsigned int load)
{
	struct tk_cpu_dbs_info_s *dbs_info = &per_cpu(tk_cpu_dbs_info, cpu);
	...
	if (load > tk_tuners->up_threshold) {
		...
		if (!tk_tuners->powersave_bias) {
			...
			int freq = tk_ops.powersave_bias_target(policy, freq_next,
					CPUFREQ_RELATION_L);
...
static void tk_dbs_timer(struct work_struct *work)
{
	...
	if (!need_load_eval(&core_dbs_info->cdbs, tk_tuners->sampling_rate)) {
		modify_all = false;
		goto max_delay;
	}
		...
		unsigned int new_rate)
{
	...
	for_each_online_cpu(cpu) {
		...
		if (!policy)
			continue;
		if (policy->governor != &cpufreq_gov_tizen_kitt) {
		...
		size_t count)
{
	...
	tizen_kitt_powersave_bias_init();
	return count;
}
...
/************************** sysfs end ************************/
struct work_struct sim_hotplug_work_kitt;
...
static int tk_init(struct dbs_data *dbs_data)
{
	...
	INIT_WORK(&sim_hotplug_work_kitt, cpu_sim_hotplug_work_kitt);
...
static
#endif
struct cpufreq_governor cpufreq_gov_tizen_kitt = {
	.name			= "tizen_kitt",
...
static int __init cpufreq_gov_dbs_init(void)
{
	...
	return cpufreq_register_governor(&cpufreq_gov_tizen_kitt);
}

static void __exit cpufreq_gov_dbs_exit(void)
{
	cpufreq_unregister_governor(&cpufreq_gov_tizen_kitt);
}
...
#ifdef CONFIG_CPU_FREQ_DEFAULT_GOV_TIZEN_KITT
