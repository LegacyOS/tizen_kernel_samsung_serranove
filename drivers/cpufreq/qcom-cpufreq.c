			...
			unsigned int index)
{
	...
	if (cpufreq_tester_en) {
		int freq_enum = cpu_freq_to_enum(new_freq);
...
static int cpufreq_min_qos_handler(struct notifier_block *b, unsigned long val, void *v)
{
	int ret;
	struct cpufreq_policy *policy;
...
static int cpufreq_max_qos_handler(struct notifier_block *b, unsigned long val, void *v)
{
	int ret;
	struct cpufreq_policy *policy;
