	...
	int iSensorType, int64_t dNewDelay)
{
	...
	case ADD_SENSOR_STATE:
		...
		if (iSensorType == TEMPERATURE_HUMIDITY_SENSOR &&
			dMsDelay == 10000)
			ssp_infof("Skip Add Temphumidity Sensor");
		else
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (unlikely(data->bSspShutdown)) {
		ssp_errf("stop sending cal data(shutdown)");
		return -EBUSY;
	}
...
static ssize_t show_pir_delay(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct ssp_data *data = dev_get_drvdata(dev);

	return snprintf(buf, PAGE_SIZE, "%lld\n", data->adDelayBuf[PIR_SENSOR]);
}

static ssize_t set_pir_delay(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	int64_t dNewDelay;
	struct ssp_data *data = dev_get_drvdata(dev);
	...
	change_sensor_delay(data, PIR_SENSOR, dNewDelay);
...
static DEVICE_ATTR(gesture_poll_delay, S_IRUGO | S_IWUSR | S_IWGRP,
...
#endif
#ifdef CONFIG_SENSORS_SSP_PIR_SENSOR
static DEVICE_ATTR(pir_standard_poll_delay, S_IRUGO | S_IWUSR | S_IWGRP,
	show_pir_delay, set_pir_delay);
static DEVICE_ATTR(pir_long_poll_delay, S_IRUGO | S_IWUSR | S_IWGRP,
	show_pir_delay, set_pir_delay);
#endif
...
#endif
#ifdef CONFIG_SENSORS_SSP_PIR_SENSOR
	&dev_attr_pir_standard_poll_delay,
	&dev_attr_pir_long_poll_delay,
#endif
...
static struct device_attribute dev_attr_input_pressure_poll_delay
	= __ATTR(poll_delay, S_IRUGO | S_IWUSR | S_IWGRP,
	show_pressure_delay, set_pressure_delay);
static struct device_attribute dev_attr_input_gesture_poll_delay
	= __ATTR(poll_delay, S_IRUGO | S_IWUSR | S_IWGRP,
	show_gesture_delay, set_gesture_delay);
static struct device_attribute dev_attr_input_light_poll_delay
	= __ATTR(poll_delay, S_IRUGO | S_IWUSR | S_IWGRP,
	show_light_delay, set_light_delay);
static struct device_attribute dev_attr_input_prox_poll_delay
	= __ATTR(poll_delay, S_IRUGO | S_IWUSR | S_IWGRP,
	show_prox_delay, set_prox_delay);
...
static struct device_attribute dev_attr_input_uv_poll_delay
	= __ATTR(poll_delay, S_IRUGO | S_IWUSR | S_IWGRP,
	show_uv_delay, set_uv_delay);
static struct device_attribute dev_attr_input_pir_poll_delay
	= __ATTR(poll_delay, S_IRUGO | S_IWUSR | S_IWGRP,
	show_pir_delay, set_pir_delay);
...
static void remove_input_poll_delay_sysfs(struct ssp_data *data)
{
	if (data->pir_standard_input_dev)
		device_remove_file(&data->pir_standard_input_dev->dev,
			&dev_attr_input_pir_poll_delay);
	if (data->pir_long_input_dev)
		device_remove_file(&data->pir_long_input_dev->dev,
		&dev_attr_input_pir_poll_delay);
	...
	if (data->prox_input_dev)
		device_remove_file(&data->prox_input_dev->dev,
			&dev_attr_input_prox_poll_delay);
	...
	if (data->gesture_input_dev)
		device_remove_file(&data->gesture_input_dev->dev,
			&dev_attr_input_gesture_poll_delay);
...
static int initialize_input_poll_delay_sysfs(struct ssp_data *data)
{
	...
	if (data->gesture_input_dev) {
		iRet = device_create_file(&data->gesture_input_dev->dev,
			&dev_attr_input_gesture_poll_delay);
		if (iRet < 0)
			goto err_create_poll_delay;
	}
	...
	if (data->prox_input_dev) {
		iRet = device_create_file(&data->prox_input_dev->dev,
				&dev_attr_input_prox_poll_delay);
		if (iRet < 0)
			goto err_create_poll_delay;
	}
	...
	if (data->pir_standard_input_dev) {
		iRet = device_create_file(&data->pir_standard_input_dev->dev,
				&dev_attr_input_pir_poll_delay);
		if (iRet < 0)			
			goto err_create_poll_delay;
	}
	...
	if (data->pir_long_input_dev) {
		iRet = device_create_file(&data->pir_long_input_dev->dev,
				&dev_attr_input_pir_poll_delay);
		if (iRet < 0)
			goto err_create_poll_delay;
	}
				...
				unsigned long arg)
{
	...
	if (batch.timeout) { /* add or dry */
		...
		if (!(batch.flag & SENSORS_BATCH_DRY_RUN)) {
			...
			enable will be called. MCU return fail
			when receive chage delay inst
...
#endif
#ifdef CONFIG_SENSORS_SSP_PIR_SENSOR
	initialize_pir_factorytest(data);
#endif
