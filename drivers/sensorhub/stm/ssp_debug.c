...
{
	u16 length = 0;
	u8 length_low = pchRcvDataFrame[(*pDataIdx)++];
	u8 length_high = pchRcvDataFrame[(*pDataIdx)++];
	int cur = *pDataIdx;
...
void sync_sensor_state(struct ssp_data *data)
{
	...
	if (set_hrm_calibration(data) < 0)
		ssp_errf("set_hrm_cal failed");
#endif

#ifdef CONFIG_SENSORS_SSP_PRESSURE_SENSOR
	if (set_pressure_cal(data) < 0)
		ssp_errf("set_pressure_cal failed");
#endif

#ifdef CONFIG_SENSORS_SSP_GYRO_SENSOR
	if (set_gyro_cal(data) < 0)
...
static void print_sensordata(struct ssp_data *data, unsigned int uSensor)
{
	...
	case LIGHT_SENSOR:
		...
		ssp_info("%u : %u, %u (%ums)", uSensor,
			data->buf[uSensor].raw_high, data->buf[uSensor].raw_low,
			get_msdelay(data->adDelayBuf[uSensor]));
#elif defined(CONFIG_SENSORS_SSP_CM36686)
		ssp_info("%u : %u, %u, %u(%ums)", uSensor,
			data->buf[uSensor].als, data->buf[uSensor].white,
			data->lux, get_msdelay(data->adDelayBuf[uSensor]));
#else
		ssp_info("%u : %u, %u, %u, %u(%ums)", uSensor,
	...
	case PROXIMITY_SENSOR:
		...
		break;
	case PROXIMITY_RAW:
		ssp_info("%u : %d(%ums)", uSensor,
			data->buf[uSensor].prox_raw[0],
			get_msdelay(data->adDelayBuf[uSensor]));
		break;
	...
	case GYRO_UNCALIB_SENSOR:
		...
		break;
	case BIO_HRM_RAW:
		ssp_info("%u : %d, %d, %d, %d, %d, %d, %d, %d, %d, %d (%ums)",
			uSensor, data->buf[uSensor].ch_a_sum ,
			data->buf[uSensor].ch_a_x1, data->buf[uSensor].ch_a_x2,
			data->buf[uSensor].ch_a_y1, data->buf[uSensor].ch_a_y2,
			data->buf[uSensor].ch_b_sum, data->buf[uSensor].ch_b_x1,
			data->buf[uSensor].ch_b_x2, data->buf[uSensor].ch_b_y1 ,
			data->buf[uSensor].ch_b_y2,
			get_msdelay(data->adDelayBuf[uSensor]));
		break;
	case BIO_HRM_RAW_FAC:
		ssp_info("%u : %d %d %d %d %d %d %d (%ums)",
			uSensor, data->buf[uSensor].frequency,
			data->buf[uSensor].green_dc_level,
			data->buf[uSensor].green_high_dc_level,
			data->buf[uSensor].green_mid_dc_level,
			data->buf[uSensor].red_dc_levet,
			data->buf[uSensor].ir_dc_level,
			data->buf[uSensor].noise_level,
			get_msdelay(data->adDelayBuf[uSensor]));
		break;
	case BIO_HRM_LIB:
		ssp_info("%u : %d, %d, %d (%ums)", uSensor,
	...
	case UV_SENSOR:
		ssp_info("%u : %d(%ums)", uSensor,
			data->buf[uSensor].uv_raw,
			get_msdelay(data->adDelayBuf[uSensor]));
		break;
	case PIR_SENSOR:
		ssp_info("%u : %d(%ums)", uSensor,
			data->buf[uSensor].event,
			get_msdelay(data->adDelayBuf[uSensor]));
		break;
	case PIR_SENSOR_RAW:
		ssp_info("%u : %d, %d, %d, %d(%ums)", uSensor,
			data->buf[uSensor].near_temp, data->buf[uSensor].near_adc,
			data->buf[uSensor].far_temp, data->buf[uSensor].far_adc,
			get_msdelay(data->adDelayBuf[uSensor]));
		break;
	default:
...
static void debug_work_func(struct work_struct *work)
{
	...
	ssp_infof("(%u) - Sensor state: 0x%x, RC: %u, CC: %u DC: %u TC: %u",
