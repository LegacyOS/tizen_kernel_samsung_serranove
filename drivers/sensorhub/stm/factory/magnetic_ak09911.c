...
Retry_selftest:
	...
	if ((iSF_X >= GM_SELFTEST_X_SPEC_MIN)
		...
		ssp_infof("x passed self test, expect -30<=x<=30");
	else
		ssp_infof("x failed self test, expect -30<=x<=30");
	if ((iSF_Y >= GM_SELFTEST_Y_SPEC_MIN)
		&& (iSF_Y <= GM_SELFTEST_Y_SPEC_MAX))
		ssp_infof("y passed self test, expect -30<=y<=30");
	else
		ssp_infof("y failed self test, expect -30<=y<=30");
	if ((iSF_Z >= GM_SELFTEST_Z_SPEC_MIN)
		&& (iSF_Z <= GM_SELFTEST_Z_SPEC_MAX))
		ssp_infof("z passed self test, expect -400<=z<=-50");
	else
		ssp_infof("z failed self test, expect -400<=z<=-50");
...
static int set_pdc_matrix(struct ssp_data *data)
{
	...
	msg = kzalloc(sizeof(*msg), GFP_KERNEL);
	if (msg == NULL) {
		ssp_errf("failed to alloc memory for ssp msg");
		return -ENOMEM;
}
	...
	if (msg->buffer == NULL) {
		ssp_errf("failed to alloc memory");
		kfree(msg);
		return -ENOMEM;
	}
...
int get_fuserom_data(struct ssp_data *data)
{
	...
	ssp_infof("FUSE ROM Data %d , %d, %d", data->uFuseRomData[0],
...
int initialize_magnetic_sensor(struct ssp_data *data)
{
	...
	if (ret < 0)
		ssp_errf("get_fuserom_data failed %d", ret);

	ret += set_pdc_matrix(data);
	if (ret < 0)
		ssp_errf("set_magnetic_pdc_matrix failed %d", ret);
	...
	struct device_attribute *attr, char *buf)
{
	...
	ret = ssp_spi_sync(data, msg, 1000);
	if (ret != SUCCESS) {
		ssp_errf("Magnetic logging data Timeout!! %d", ret);
		goto exit;
	}
...
{
	sensors_register(data->devices[GEOMAGNETIC_SENSOR], data, mag_attrs,
			"magnetic_sensor");
}

void remove_magnetic_factorytest(struct ssp_data *data)
{
	sensors_unregister(data->devices[GEOMAGNETIC_SENSOR], mag_attrs);
}
