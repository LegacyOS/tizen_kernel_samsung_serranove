...
#define DEF_BIAS_LSB_THRESH_SELF_9250	(30 * DEF_GYRO_SENS)
...
short mpu9250_gyro_get_temp(struct ssp_data *data)
{
	char chTempBuf[2] = { 0};
	unsigned char reg[2];
	short temperature = 0;
	...
	struct device_attribute *attr, char *buf)
{
	...
	temperature = mpu9250_gyro_get_temp(data);
	return snprintf(buf, PAGE_SIZE, "%d\n", temperature);
}

u32 mpu9250_selftest_sqrt(u32 sqsum)
{
	u32 sq_rt;
	u32 g0, g1, g2, g3, g4;
	u32 seed;
...
static ssize_t mpu9250_gyro_selftest(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	char chTempBuf[36] = { 0,};
	u8 initialized = 0;
	...
	int dps_rms[3] = { 0, };
	u32 temp = 0;
	int bias_thresh = DEF_BIAS_LSB_THRESH_SELF_9250;
	struct ssp_data *data = dev_get_drvdata(dev);
	...
	for (i = 0; i < 3; i++) {
		...
		dps_rms[i] = mpu9250_selftest_sqrt(temp) / DEF_GYRO_SENS;
...
static DEVICE_ATTR(selftest, S_IRUGO, mpu9250_gyro_selftest, NULL);
