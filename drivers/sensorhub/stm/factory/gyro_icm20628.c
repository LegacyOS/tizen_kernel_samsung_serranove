...
#define DEF_BIAS_LSB_THRESH_SELF_20628	(40 * DEF_GYRO_SENS)
...
int gyro_open_calibration(struct ssp_data *data)
{
	...
	ssp_infof("open gyro calibration %d, %d, %d",
...
static int save_gyro_caldata(struct ssp_data *data, s16 *iCalData)
{
	...
	if (data->uGyroDps == GYROSCOPE_DPS250)
		scale_factor = 3;
	else if (data->uGyroDps == GYROSCOPE_DPS500)
		scale_factor = 2;
	else if (data->uGyroDps == GYROSCOPE_DPS1000)
		scale_factor = 1;
	else
		scale_factor = 0;

	data->gyrocal.x = iCalData[0] << scale_factor;
	data->gyrocal.y = iCalData[1] << scale_factor;
	data->gyrocal.z = iCalData[2] << scale_factor;
...
short icm20628_gyro_get_temp(struct ssp_data *data)
{
	char chTempBuf[2] = { 0};
	unsigned char reg[2];
	short temperature = 0;
	...
	struct device_attribute *attr, char *buf)
{
	...
	temperature = icm20628_gyro_get_temp(data);
	return snprintf(buf, PAGE_SIZE, "%d\n", temperature);
}

u32 icm20628_selftest_sqrt(u32 sqsum)
{
	u32 sq_rt;
	u32 g0, g1, g2, g3, g4;
	u32 seed;
...
static ssize_t icm20628_gyro_selftest(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	char chTempBuf[36] = { 0,};
	u8 initialized = 0;
	s8 hw_result = 0;
	s8 hw_selft_result = 0;
	...
	int dps_rms[3] = { 0, };
	u32 temp = 0;
	int bias_thresh = DEF_BIAS_LSB_THRESH_SELF_20628;
	struct ssp_data *data = dev_get_drvdata(dev);
	...
	ssp_errf("%d %d %d %d %d %d %d %d %d %d %d %d", chTempBuf[0],
	...
	for (i = 0; i < 3; i++) {
		...
		dps_rms[i] = icm20628_selftest_sqrt(temp) / DEF_GYRO_SENS;
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	else if (iNewDps == GYROSCOPE_DPS1000)
		msg->options |= 2 << SSP_GYRO_DPS;
	else if (iNewDps == GYROSCOPE_DPS2000)
		msg->options |= 3 << SSP_GYRO_DPS;
	else {
		msg->options |= 3 << SSP_GYRO_DPS;
		iNewDps = GYROSCOPE_DPS2000;
	}
...
static DEVICE_ATTR(selftest, S_IRUGO, icm20628_gyro_selftest, NULL);
