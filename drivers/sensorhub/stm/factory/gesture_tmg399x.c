		...
		struct device_attribute *attr, char *buf)
{
	...
	if (iRet != SUCCESS) {
		ssp_errf("Gesture Selftest Timeout!!");
		goto exit;
	}
	...
	ssp_infof("self test A = %d, B = %d, C = %d, D = %d",
	...
	struct device_attribute *attr, char *buf)
{
	...
	ssp_infof("Ir_Current Setting = %d", data->uIr_Current);
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	ssp_infof("new Ir_Current Setting : %d", data->uIr_Current);
