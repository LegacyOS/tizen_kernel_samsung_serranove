...
static int hrm_open_oscregvalue(struct ssp_data *data)
{
	...
	ssp_infof("%d", data->hrmcal.osc_reg);
...
static int hrm_open_eol_data(struct ssp_data *data)
{
	...
	ssp_infof("[%d %d %d %d %d]", eol_data[0], eol_data[1],
		eol_data[2], eol_data[3], eol_data[4]);
	ssp_infof("[%d %d %d %d %d]", eol_data[5], eol_data[6],
		eol_data[7], eol_data[8], eol_data[9]);
	ssp_infof("[%d %d %d %d %d]", eol_data[10], eol_data[11],
...
static int save_hrm_oscregvalue(struct ssp_data *data)
{
	...
	if (IS_ERR(cal_filp)) {
		ssp_errf("Can't open osc_reg_value file");
		set_fs(old_fs);
		iRet = PTR_ERR(cal_filp);
		return -EIO;
	}
	...
	iRet = cal_filp->f_op->write(cal_filp, (char *)&data->hrmcal.osc_reg,
		sizeof(data->hrmcal.osc_reg), &cal_filp->f_pos);
	if (iRet != sizeof(data->hrmcal.osc_reg)) {
		ssp_errf("Can't write hrm osc_reg_value to file");
		iRet = -EIO;
	}
	...
	struct device_attribute *attr, char *buf)
{
	...
	if (iRet != SUCCESS) {
		ssp_errf("hrm lib version Timeout!!");
		goto exit;
	}
	...
	struct device_attribute *attr, char *buf)
{
	...
	iRet = cal_filp->f_op->read(cal_filp, (char *)&eol_data,
		sizeof(eol_data), &cal_filp->f_pos);
	if (iRet != sizeof(eol_data))
		iRet = -EIO;
