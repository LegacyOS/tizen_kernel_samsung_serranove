				...
				unsigned long arg)
{
	...
	switch (cmd) {
		...
		case IOCTL_READ_ADC_BATT_DATA:
			...
			if (unlikely(ret)) {
				ssp_errf("read bluk adc1 data err(%d)", ret);
				goto ioctl_error;
			}
			break;
		...
		default:
			ssp_errf("temp ioctl cmd err(%d)", cmd);
			ret = EINVAL;
			goto ioctl_error;
	}
...
static int get_cp_thm_value(struct ssp_data *data)
{
	...
	err = qpnp_vadc_read(ssp_vadc, LR_MUX6_PU1_AMUX_THM3, &results);
		...
		if (err) {
			ssp_errf("error reading chn %d, rc = %d",
			LR_MUX6_PU2_AMUX_THM3, err);
			return err;
		}
	...
	struct device_attribute *attr, char *buf)
{
	...
	ssp_infof("engine_ver = %s_%s", MODEL_NAME, data->comp_engine_ver);

	return snprintf(buf, PAGE_SIZE, "%s_%s\n", MODEL_NAME,
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	ssp_infof("engine_ver = %s, %s", data->comp_engine_ver, buf);
	...
	struct device_attribute *attr, char *buf)
{
	...
	ssp_infof("engine_ver2 = %s_%s", MODEL_NAME, data->comp_engine_ver2);
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	ssp_infof("engine_ver2 = %s, %s", data->comp_engine_ver2, buf);
	...
	struct device_attribute *attr, char *buf)
{
	...
	adc = get_cp_thm_value(data);
	if (adc < 0) {
		ssp_errf("reading adc failed.(%d)", adc);
		temp = adc;
	} else {
		temp = convert_adc_to_temp(data, adc);
	}

	ssp_infof("cp_temperature(Celsius * 10) = %d", temp);
	return snprintf(buf, PAGE_SIZE, "%d\n", temp);
}
	...
	struct device_attribute *attr, char *buf)
{
	...
	ssp_infof("adc %d", temp_table_batt[mid].temperature);
	...
	struct device_attribute *attr, char *buf)
{
	...
	ssp_infof("adc %d", temp_table_chg[mid].temperature);
	return snprintf(buf, PAGE_SIZE, "%d\n",
	...
	struct device_attribute *attr, char *buf)
{
	...
	if (iRet != SUCCESS) {
		ssp_errf("Temphumidity check crc Timeout!! %d", iRet);
		goto exit;
	}

	ssp_infof("Check_CRC : %d", chTempBuf);
...
void initialize_temphumidity_factorytest(struct ssp_data *data)
{
	...
	if (ret < 0) {
		ssp_errf("register temphumidity misc device err(%d)", ret);
	}
	...
	if (IS_ERR(ssp_vadc)) {
		...
		if (ret != -EPROBE_DEFER)
			ssp_errf("Fail to get vadc %d", ret);
	}
