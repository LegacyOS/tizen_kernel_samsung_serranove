...
int set_accel_cal(struct ssp_data *data)
{
	...
	ssp_infof("Set accel cal data %d, %d, %d",
...
static int accel_do_calibrate(struct ssp_data *data, int iEnable)
{
	...
	ssp_infof("do accel calibrate %d, %d, %d",
		data->accelcal.x, data->accelcal.y, data->accelcal.z);
	...
	iRet = cal_filp->f_op->write(cal_filp, (char *)&data->accelcal,
		3 * sizeof(int), &cal_filp->f_pos);
	if (iRet != 3 * sizeof(int)) {
		ssp_errf("Can't write the accelcal to file");
		iRet = -EIO;
	}
	...
	struct device_attribute *attr, char *buf)
{
	...
	if (iRet < 0)
		ssp_errf("calibration open failed(%d)", iRet);

	ssp_infof("Cal data : %d %d %d - %d",
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (iRet < 0)
		ssp_errf("accel_do_calibrate() failed");
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (sysfs_streq(buf, "1"))
		...
		if (iRet != SUCCESS) {
			ssp_errf("accel Selftest Timeout!!");
			goto exit;
		}
	...
	struct device_attribute *attr, char *buf)
{
	...
	iRet = ssp_spi_sync(data, msg, 3000);
	if (iRet != SUCCESS) {
		ssp_errf("accel hw selftest Timeout!!");
		goto exit;
	}
	...
	ssp_infof("%d, %d, %d, %d, %d", init_status, result,
