	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (data->sealevelpressure == 0) {
		ssp_infof("our->temperature = 0");
		data->sealevelpressure = -1;
	}

	ssp_infof("sea_level_pressure = %d", data->sealevelpressure);
	return size;
}
...
int pressure_open_calibration(struct ssp_data *data)
{
	...
	if (IS_ERR(cal_filp)) {
		...
		if (iErr != -ENOENT)
			ssp_errf("Can't open calibration file(%d)", iErr);
		set_fs(old_fs);
		return iErr;
	}
	...
	if (iErr < 0) {
		ssp_errf("Can't read the cal data from file (%d)", iErr);
		return iErr;
	}
	...
	if (iErr < 0) {
		ssp_errf("kstrtoint failed. %d", iErr);
		return iErr;
	}

	ssp_infof("open barometer calibration %d", data->iPressureCal);

	if (data->iPressureCal < PR_ABS_MIN || data->iPressureCal > PR_ABS_MAX)
		ssp_errf("wrong offset value!!!");
...
int set_pressure_cal(struct ssp_data *data)
{
	...
	ssp_info("Set pressure cal data %d", pressure_cal);
