...
#if defined(CONFIG_MUIC_SUPPORT_DESKDOCK) || defined(CONFIG_USB_NOTIFY_LAYER) ||\
...
static int sm5703_muic_attach_dev(struct sm5703_muic_usbsw *usbsw)
{
	...
	/* NON STANDARD CHARGER */
		...
		pr_info("[MUIC] Non Standard Charger Connected\n");

		usbsw->attached_dev = ATTACHED_DEV_TA_MUIC;
		pdata->callback(CABLE_TYPE_NON_STND_AC, SM5703_MUIC_ATTACHED);
	/* CHARGER */
	} else if ((val1 & DEV_T1_CHARGER_MASK) ||
			(val3 & DEV_U200_CHARGER)) {
		...
		const struct i2c_device_id *id)
{
	...
	if (ret < 0)
		dev_err(&client->dev, "failed to read MANUAL SW1 Reg, err:%d\n",
				ret);
	dev_info(&client->dev, "REG_MANUAL_SW1=%d\n",
				ret);
