...
#ifdef CONFIG_SLEEP_MONITOR
u64 ref_time[2];
u64 ref_accumulated[2];

int rpm_stats_sleep_monitor_read(void *priv, unsigned int *raw_val, unsigned int mode_idx)
{
	struct msm_rpmstats_platform_data *pdata;
	void __iomem *reg_base = NULL;
	u32 count;
	u64 accumulated, accumulated_temp;
	u64 new_time;
	...
	count = msm_rpmstats_read_long_register_v2(reg_base, mode_idx,
			offsetof(struct msm_rpm_stats_data_v2, count));

	accumulated = msm_rpmstats_read_quad_register_v2(reg_base,
			mode_idx, offsetof(struct msm_rpm_stats_data_v2,
				accumulated));

	accumulated_temp = accumulated;

	*raw_val = (count << 20) | (0x000fffff & get_time_in_sec(accumulated));

	iounmap(reg_base);

	/* Calculate pretty value */
	if (mode_idx == 1) {
		/* Check pretty value for VMIN */
		new_time = arch_counter_get_cntpct();

		accumulated -= ref_accumulated[mode_idx];
		do_div(accumulated, (new_time - ref_time[mode_idx])>> 4);

		ref_accumulated[mode_idx] = accumulated_temp;
		ref_time[mode_idx] = new_time;

		return 0xf - (accumulated & 0xf);
	}
	return 0;
}


int rpm_stats_xosd_sleep_monitor_read(void *priv, unsigned int *raw_val,
		int check_level, int caller_type)
{
	return rpm_stats_sleep_monitor_read(priv, raw_val, 0);
}

int rpm_stats_vmin_sleep_monitor_read(void *priv, unsigned int *raw_val,
		int check_level, int caller_type)
{
	return rpm_stats_sleep_monitor_read(priv, raw_val, 1);
}

static struct sleep_monitor_ops rpm_stats_xosd_sleep_monitor_ops = {
	.read_cb_func = rpm_stats_xosd_sleep_monitor_read,
};
static struct sleep_monitor_ops rpm_stats_vmin_sleep_monitor_ops = {
	.read_cb_func = rpm_stats_vmin_sleep_monitor_read,
};
#endif
...
#ifdef CONFIG_SLEEP_MONITOR
	if (pdata->version == 2) {
		sleep_monitor_register_ops(pdata, &rpm_stats_xosd_sleep_monitor_ops, SLEEP_MONITOR_RPM_XOSD);
		sleep_monitor_register_ops(pdata, &rpm_stats_vmin_sleep_monitor_ops, SLEEP_MONITOR_RPM_VMIN);
	}
#endif
