...
#ifdef CONFIG_SLEEP_MONITOR
int rpm_ee_sleep_monitor_read(void *priv, unsigned int *raw_val, char * ee_name)
{
	struct msm_rpm_master_stats_platform_data *pdata;
	unsigned int master_cnt;
	uint32_t active_cores;
	uint32_t numshutdowns;
	...
	reg_base = ioremap(pdata->phys_addr_base, pdata->phys_size);
	...
	/* Find valid index for given  master */
	for (master_cnt = 0; master_cnt < pdata->num_masters; master_cnt++) {
		dev_len[0] = strlen(ee_name);
		dev_len[1] = strlen(pdata->masters[master_cnt]);
		if(!strncmp(ee_name, pdata->masters[master_cnt], min(dev_len[0], dev_len[1])))
			break;
	}

	/* If ee_name is not exist, just return */
	if (master_cnt >= pdata->num_masters) {
		iounmap(reg_base);
		return -1;
	}

	// TODO: Need to check whether locking is required for below resources
	// TODO:  In msm_rpm_master_copy_stats, locking is used.

	/* Get information of each ee */
	active_cores = readl_relaxed(reg_base + (master_cnt * pdata->master_offset) +
		offsetof(struct msm_rpm_master_stats, active_cores));

	numshutdowns = readl_relaxed(reg_base + (master_cnt * pdata->master_offset +
		 offsetof(struct msm_rpm_master_stats, numshutdowns)));

	/* raw_val = (active_cores << 24) | (0x00ffffff & numshutdowns); */
	*raw_val = (active_cores << 28) | (0x0fffffff & numshutdowns);

	iounmap(reg_base);

	return (0xf & active_cores);
}

int rpm_apss_sleep_monitor_read(void *priv, unsigned int *raw_val,
		int check_level, int caller_type)
{
	return rpm_ee_sleep_monitor_read(priv, raw_val, "APSS");
}

int rpm_mpss_sleep_monitor_read(void *priv, unsigned int *raw_val,
		int check_level, int caller_type)
{
	return rpm_ee_sleep_monitor_read(priv, raw_val, "MPSS");
}

int rpm_pron_sleep_monitor_read(void *priv, unsigned int *raw_val,
		int check_level, int caller_type)
{
	return rpm_ee_sleep_monitor_read(priv, raw_val, "PRON");
}

static struct sleep_monitor_ops rpm_apss_sleep_monitor_ops = {
	.read_cb_func = rpm_apss_sleep_monitor_read,
};

static struct sleep_monitor_ops rpm_mpss_sleep_monitor_ops = {
	.read_cb_func = rpm_mpss_sleep_monitor_read,
};

static struct sleep_monitor_ops rpm_pron_sleep_monitor_ops = {
	.read_cb_func = rpm_pron_sleep_monitor_read,
};
#endif
...
#ifdef CONFIG_SLEEP_MONITOR
	if (pdata->version == 2) {
		sleep_monitor_register_ops(pdata, &rpm_apss_sleep_monitor_ops, SLEEP_MONITOR_EE_APSS);
		sleep_monitor_register_ops(pdata, &rpm_mpss_sleep_monitor_ops, SLEEP_MONITOR_EE_MPSS);
		sleep_monitor_register_ops(pdata, &rpm_pron_sleep_monitor_ops, SLEEP_MONITOR_EE_PRON);
	}
#endif
