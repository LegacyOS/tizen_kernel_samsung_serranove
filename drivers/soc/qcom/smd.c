 ...
 */
const char *smd_irq_to_subsystem(unsigned int irq)
{
	int i;
	...
	for (i=0; i < ARRAY_SIZE(edge_to_pids); i++) {
		int_cfg = &private_intr_config[edge_to_pids[i].remote_pid].smd;
		if (int_cfg->irq_id == irq) {
			if (edge_to_pids[i].remote_pid == SMD_RPM) {
				subsys = "rpm";
				break;
			}
...
EXPORT_SYMBOL(smd_irq_to_subsystem);
#endif
