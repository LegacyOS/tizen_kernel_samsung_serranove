...
int clock_debug_measure_get_extern(void *data, u64 *val);
unsigned int get_bimc_clk(void)
{
	u64 data;
	int ret;
	ret =clock_debug_measure_get_extern(&(bimc_clk.c) , &data);
	if(ret<0)
	pr_err("%s --cannot measure clock rate ",__func__);
...
unsigned int get_snoc_clk(void)
{
	u64 data;
	int ret;
	ret=clock_debug_measure_get_extern(&(snoc_clk.c) , &data);
	if(ret<0)
	pr_err("%s --cannot measure clock rate ",__func__);
	return (unsigned int)data;
}
