...
static int msm8x16_headset_mic_event(struct snd_soc_dapm_widget *w,
			      struct snd_kcontrol *kcontrol, int event);
...
static const struct snd_soc_dapm_widget msm8x16_dapm_widgets[] = {
	...
	SND_SOC_DAPM_MIC("Handset Mic", msm8x16_headset_mic_event),
...
static int msm_quat_mi2s_be_hw_params_fixup(struct snd_soc_pcm_runtime *rtd,
				struct snd_pcm_hw_params *params)
{
	struct snd_interval *rate = hw_param_interval(params,
					SNDRV_PCM_HW_PARAM_RATE);
	...
	struct snd_interval *channels = hw_param_interval(params,
					SNDRV_PCM_HW_PARAM_CHANNELS);
	int set_rate = CONFIG_SND_SOC_MSM8X16_QUAT_MI2S_RATE;
	...
	case 48000:
		...
		break;
	default:
		pr_err("%s() - not supported rate %d\n", __func__, set_rate);
		set_rate = 48000; /* default */
		break;
	}

	rate->min = rate->max = set_rate;
...
{
	switch (ucontrol->value.integer.value[0]) {
	case 0: /* alsa lib set index */
	case 8000:
		msm_btsco_rate = BTSCO_RATE_8KHZ;
		break;
	case 1: /* alsa lib set index */ 
	case 16000:
...
static int msm8x16_headset_mic_event(struct snd_soc_dapm_widget *w,
			      struct snd_kcontrol *kcontrol, int event)
{
	/*Fix noise when start to record*/
	switch (event) {
...
static struct snd_soc_dai_link msm8x16_dai[] = {
	...
	},
	{
		...
		.no_pcm = 1,
		.be_id = MSM_BACKEND_DAI_QUATERNARY_MI2S_RX,
		.be_hw_params_fixup = msm_quat_mi2s_be_hw_params_fixup,
...
void disable_mclk(struct work_struct *work)
{
	...
	pdata = container_of(dwork, struct msm8916_asoc_mach_data,
				disable_mclk_work);
	mutex_lock(&pdata->cdc_mclk_mutex);
	pr_info("%s: mclk_enabled %d mclk_rsc_ref %d\n", __func__,
			atomic_read(&pdata->mclk_enabled),
			atomic_read(&pdata->mclk_rsc_ref));

	if (atomic_read(&pdata->mclk_enabled) == true
		&& atomic_read(&pdata->mclk_rsc_ref) == 0) {
		pr_info("Disable the mclk\n");
		pdata->digital_cdc_clk.clk_val = 0;
		ret = afe_set_digital_codec_core_clock(
				AFE_PORT_ID_PRIMARY_MI2S_RX,
				&pdata->digital_cdc_clk);
...
static int msm8x16_asoc_machine_probe(struct platform_device *pdev)
{
	...
	if (!strcmp(ptr, "external")) {
		...
		if (ret < 0) {
			pr_err("%s: failed to get the codec gpio's %d\n",
					__func__, ret);
			goto err;
		}
#ifdef CONFIG_SND_SOC_MSM_TIZEN_TUNE
		/* set default pinctrl as suspend */
		ret = pinctrl_select_state(pinctrl,
					pinctrl_info.cdc_lines_sus);
		if (ret < 0) {
			pr_err("%s: failed to set init codec gpios %d\n",
					__func__, ret);
			goto err;
		} else {
			pr_info("%s: init codec gpio set success\n", __func__);
		}
#endif
	}
