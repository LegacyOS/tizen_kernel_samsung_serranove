...
/* 19.2MHz */
#define MSM_ARIZONA_MCLK1_RATE	19200000
/* 98.304 MHz */
#define MSM_ARIZONA_SYSCLK_RATE	98304000
/* 294.912 MHz */
#define MSM_ARIZONA_FLL_RATE		(3 * MSM_ARIZONA_SYSCLK_RATE)
/* 147.456 MHz */
#define MSM_CLEARWATER_DSPCLK_RATE	(MSM_ARIZONA_FLL_RATE / 2)

#define DRV_NAME "msm8x16-asoc-arizona"
#else
...
#ifdef CONFIG_SND_SOC_MSM8X16_MBHC
...
#ifdef CONFIG_SND_SOC_MSM8X16_MI2S_SLAVE
static struct afe_clk_cfg mi2s_slave_clk = {
	AFE_API_VERSION_I2S_CONFIG,
	0,
...
#ifdef CONFIG_SND_SOC_MSM8X16_MI2S_SLAVE
		if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
			mi2s_slave_clk.clk_val1 = Q6AFE_LPASS_IBIT_CLK_DISABLE;
					...
					bool dapm)
{
	...
	if (enable) {
		if (atomic_inc_return(&pdata->mclk_rsc_ref) == 1) {
			/* if primary mi2s has disabled from pinctrol of tlmm.
			   we can remove this */
...
#ifndef CONFIG_SND_SOC_MSM8X16_ARIZONA
...
#ifdef CONFIG_SND_SOC_MSM8X16_ARIZONA
static const struct snd_kcontrol_new wcnss_snd_controls[] = {
	SOC_ENUM_EXT("Internal BTSCO SampleRate", msm_btsco_enum[0],
		     msm_btsco_rate_get, msm_btsco_rate_put),
};
#endif
...
static void msm_mi2s_snd_shutdown(struct snd_pcm_substream *substream)
{
	...
	if (!pdata->codec_type) {
...
#ifdef CONFIG_SND_SOC_MSM8X16_ARIZONA
		mutex_lock(&pdata->ext_tlmm_mutex);
		if (atomic_dec_return(&pdata->ext_tlmm_ref) == 0) {
#endif
		...
		mutex_unlock(&pdata->ext_tlmm_mutex);
#endif
		ret =  msm_snd_enable_codec_ext_clk(codec, 0, false);
		if (ret < 0) {
...
static int msm_mi2s_snd_startup(struct snd_pcm_substream *substream)
{
	...
	if (!pdata->codec_type) {
		...
		/*iounmap(vaddr);*/
		val = val | 0x00000002;
		pr_debug("%s: LPASS_CSR_GP_IO_MUX_SPKR_CTL 0x%x\n",
							__func__, val);
		/*vaddr = ioremap(LPASS_CSR_GP_IO_MUX_SPKR_CTL , 4);
		if (!vaddr) {
			pr_err("%s ioremap failure for addr %x",
		...
		/*iounmap(vaddr);*/
#ifdef CONFIG_SND_SOC_MSM8X16_MI2S_SLAVE
		val = val | 0x00420000; /* Need to check */
#else
		val = val | 0x0002000E;
#endif
		pr_debug("%s: LPASS_CSR_GP_IO_MUX_MIC_CTL 0x%x\n",
							__func__, val);
		/*vaddr = ioremap(LPASS_CSR_GP_IO_MUX_MIC_CTL , 4);
		if (!vaddr) {
			pr_err("%s ioremap failure for addr %x",
...
#ifdef CONFIG_SND_SOC_MSM8X16_ARIZONA
		mutex_lock(&pdata->ext_tlmm_mutex);
		if (atomic_inc_return(&pdata->ext_tlmm_ref) == 1) {
#endif
			ret = pinctrl_select_state(ext_cdc_pinctrl_info.pinctrl,
						ext_cdc_pinctrl_info.tlmm_act);
			if (ret < 0) {
...
#ifdef CONFIG_SND_SOC_MSM8X16_ARIZONA
#ifdef CONFIG_SND_SOC_MSM8X16_MI2S_SLAVE
	codec_dai_fmt = SND_SOC_DAIFMT_I2S | SND_SOC_DAIFMT_NB_NF
						| SND_SOC_DAIFMT_CBM_CFM;
#else
	codec_dai_fmt = SND_SOC_DAIFMT_I2S | SND_SOC_DAIFMT_NB_NF
						| SND_SOC_DAIFMT_CBS_CFS;
#endif
	dev_info(codec_dai->dev, "%s: codec_dai_fmt %s\n", __func__,
			codec_dai_fmt & SND_SOC_DAIFMT_CBM_CFM ? \
							"master" : "slave");
...
#ifdef CONFIG_SND_SOC_MSM8X16_ARIZONA
static int get_arizona_rtd_num(struct snd_soc_card *card)
{
	int i;
	static int arizona_rtd_num = -ENODEV;
	char codec_str[32] = { '\0' };
	struct msm8916_asoc_mach_data *pdata = snd_soc_card_get_drvdata(card);

	if (pdata->arizona_type == WM5110)
		strlcpy(codec_str, "florida-codec", sizeof(codec_str));
	else if (pdata->arizona_type == WM1840)
		strlcpy(codec_str, "clearwater-codec", sizeof(codec_str));
	else
		return -ENODEV;

	if (arizona_rtd_num == -ENODEV) {
		/* let's find */
		for (i = 0; i < card->num_rtd; i++) {
			dev_dbg(card->dev, "%s: codec_name %s\n",
					__func__, card->rtd[i].codec->name);
			if (!strcmp(codec_str, card->rtd[i].codec->name)) {
				dev_info(card->dev,
					"FOUND %d %s\n", i, codec_str);
				break;
			}
		...
		if (i == card->num_rtd)
			dev_info(card->dev, "can't find %s\n", codec_str);
		else
			arizona_rtd_num = i;
	}

	return arizona_rtd_num;

}

static int msm8x16_arizona_set_bias_level(struct snd_soc_card *card,
					struct snd_soc_dapm_context *dapm,
					enum snd_soc_bias_level level)
{
...
#ifdef CONFIG_SND_SOC_MSM8X16_MI2S_SLAVE
	struct msm8916_asoc_mach_data *pdata = snd_soc_card_get_drvdata(card);
	int arizona_fll1_refclk_id = 0;
	int arizona_fll1_id = 0;
	int ret;
#endif

	/* find arizona rtd num */
	num = get_arizona_rtd_num(card);
	if (num == -ENODEV)
		return 0;

	codec_dai = card->rtd[num].codec_dai;
	codec = codec_dai->codec;

	/* make sure it is arizona */
	if (dapm->dev != codec_dai->dev)
		return 0;

#ifdef CONFIG_SND_SOC_MSM8X16_MI2S_SLAVE
	if (pdata->arizona_type == WM5110) {
		arizona_fll1_refclk_id = FLORIDA_FLL1_REFCLK;
		arizona_fll1_id = FLORIDA_FLL1;
	} else if (pdata->arizona_type == WM1840) {
		arizona_fll1_refclk_id = CLEARWATER_FLL1_REFCLK;
		arizona_fll1_id = CLEARWATER_FLL1;
	}
	...
	case SND_SOC_BIAS_PREPARE:
		...
		ret = snd_soc_codec_set_pll(codec, arizona_fll1_refclk_id,
		...
		if (ret < 0) {
			...
			dev_info(codec->dev,
				"%s: started FLL1 REF %s\n", __func__, codec_dai->name);
		}
		/* This will give us an FLL output which is
		* post divided for SYSCLK/DSPCLK (/3, /2)
		* so we could in theory use either, but use
		* SYSCLK for consistency with other arizona
		* devices. */
		ret = snd_soc_codec_set_pll(codec, arizona_fll1_id,
					ARIZONA_FLL_SRC_MCLK1,
					MSM_ARIZONA_MCLK1_RATE,
					MSM_ARIZONA_SYSCLK_RATE);
		if (ret < 0)
			dev_err(codec->dev, "Failed to start FLL1: %d\n", ret);
		else
			dev_info(codec->dev, "%s: started FLL1 %s\n", __func__, codec_dai->name);
...
static int msm8x16_arizona_set_bias_level_post(struct snd_soc_card *card,
					struct snd_soc_dapm_context *dapm,
					enum snd_soc_bias_level level)
{
	struct snd_soc_dai *codec_dai = NULL;
	...
	case SND_SOC_BIAS_STANDBY:
		ret = snd_soc_codec_set_pll(codec, arizona_fll1_id, 0, 0, 0);
		if (ret < 0)
			dev_err(codec->dev, "Failed to stop FLL: %d\n", ret);
		else
			dev_info(codec->dev, "%s: stopped FLL %s\n", __func__, codec_dai->name);
...
static int init_voice_trigger_input_device(void)
{
	int ret = 0;
	...
	voice_input->name = "voice trigger";
	...
	if (ret < 0) {
		...
		dev_info(&voice_input->dev, "%s: registered\n", __func__);
	}
...
static int deinit_voice_trigger_input_device(void)
{
	input_unregister_device(voice_input);
	return 0;
}

static void arizona_ez2c_trigger(void)
{
...
static int msm8x16_arizona_late_probe(struct snd_soc_card *card)
{
	struct snd_soc_codec *codec = NULL;
	struct snd_soc_dai *arizona_aif1_dai = NULL;
	struct msm8916_asoc_mach_data *pdata = snd_soc_card_get_drvdata(card);
	int num, ret;
	...
	codec = card->rtd[num].codec;
	arizona_aif1_dai = card->rtd[num].codec_dai;
	...
	if (pdata->arizona_type == WM1840) {
		/* only clearwater codec needs DSPCLK setting */
		ret = snd_soc_codec_set_sysclk(codec, ARIZONA_CLK_DSPCLK,
						ARIZONA_CLK_SRC_FLL1,
						MSM_CLEARWATER_DSPCLK_RATE,
						SND_SOC_CLOCK_IN);
		if (ret != 0) {
	...
	ret = snd_soc_dai_set_sysclk(arizona_aif1_dai,
					ARIZONA_CLK_SYSCLK, 0, 0);
	if (ret != 0) {
		dev_err(arizona_aif1_dai->dev,
				"Failed to set AIF1 clock: %d\n", ret);
		return ret;
	}

	arizona_set_ez2ctrl_cb(codec, arizona_ez2c_trigger);
...
#ifdef CONFIG_SND_SOC_MSM8X16_EXT_AMP
static int ext_amp_control(struct snd_soc_dapm_widget *w,
				struct snd_kcontrol *kcontrol, int event)
{
	struct msm8916_asoc_mach_data *pdata = NULL;
	int ret = 0;
	...
	pr_info("%s: event %d\n", __func__, event);
	pdata = snd_soc_card_get_drvdata(w->codec->card);
#if defined(CONFIG_TPA2026D2)
	/* Add TPA2026D2 control here */
	tpa2026d2_spk_power(1, 1);
#else
	/* This is workaround */
	switch (event) {
	case SND_SOC_DAPM_PRE_PMU:
		gpio_set_value(pdata->ext_amp_en_gpio, 1);
		usleep_range(5000, 6000);
		break;
	case SND_SOC_DAPM_POST_PMD:
		gpio_set_value(pdata->ext_amp_en_gpio, 0);
		break;
	}
#endif
	return ret;
...
static struct snd_soc_dapm_widget msm8x16_arizona_widgets[] = {
	SND_SOC_DAPM_MIC("MIC_1", NULL),
	SND_SOC_DAPM_MIC("MIC_2", NULL),
	SND_SOC_DAPM_MIC("MIC_3", NULL),
	SND_SOC_DAPM_MIC("MIC_4", NULL),
#ifdef CONFIG_SND_SOC_MSM8X16_EXT_AMP
	SND_SOC_DAPM_SPK("EXT_SPKAMP", ext_amp_control),
#endif
};

static int msm_audrx_init_arizona(struct snd_soc_pcm_runtime *rtd)
{
	struct snd_soc_codec *codec = rtd->codec;
	struct snd_soc_dapm_context *dapm = &codec->dapm;

	dev_info(dapm->dev, "%s()\n", __func__);
	snd_soc_add_codec_controls(codec, wcnss_snd_controls,
				ARRAY_SIZE(wcnss_snd_controls));

	snd_soc_dapm_new_controls(dapm, msm8x16_arizona_widgets,
				ARRAY_SIZE(msm8x16_arizona_widgets));

	snd_soc_dapm_ignore_suspend(dapm, "MIC_1");
	snd_soc_dapm_ignore_suspend(dapm, "MIC_2");
	snd_soc_dapm_ignore_suspend(dapm, "MIC_3");
	snd_soc_dapm_ignore_suspend(dapm, "MIC_4");
...
static struct snd_soc_dai_link msm8x16_clearwater_dai[] = {
	/* Backend DAI Links */
	{
		.name = LPASS_BE_QUAT_MI2S_RX,
		.stream_name = "Quaternary MI2S Playback",
	...
	/* Voice Control DAI */
	{
		...
		.platform_name = "clearwater-platform",
...
static struct snd_soc_dai_link msm8x16_florida_dai[] = {
	...
	/* Voice Control DAI */
	{
		...
		.platform_name = "florida-platform",
...
#if defined(CONFIG_SND_SOC_MSM_QDSP6V2_COMPR)
	{/* hw:x,9 */
		.name = "MSM8X16 Compr",
		.stream_name = "COMPR",
...
#ifdef CONFIG_SND_SOC_MSM8X16_ARIZONA
static struct snd_soc_dai_link msm8x16_clearwater_dai_links[
				ARRAY_SIZE(msm8x16_dai) +
				ARRAY_SIZE(msm8x16_clearwater_dai)];
...
#ifdef CONFIG_SND_SOC_MSM8X16_ARIZONA
struct snd_soc_card snd_soc_card_clearwater_msm8916 = {
	.name		= "msm8x16-clearwater-snd-card",
	.dai_link	= msm8x16_clearwater_dai_links,
	.num_links	= ARRAY_SIZE(msm8x16_clearwater_dai_links),
};
struct snd_soc_card snd_soc_card_florida_msm8916 = {
	.name		= "msm8x16-florida-snd-card",
	.dai_link	= msm8x16_florida_dai_links,
	.num_links	= ARRAY_SIZE(msm8x16_florida_dai_links),
};
#endif
...
static struct snd_soc_card bear_cards[MAX_SND_CARDS] = {
...
#ifdef CONFIG_SND_SOC_MSM8X16_ARIZONA
	{
		...
		.late_probe	= msm8x16_arizona_late_probe,
		.set_bias_level = msm8x16_arizona_set_bias_level,
		.set_bias_level_post =
				msm8x16_arizona_set_bias_level_post,
	},
	{
		.name		= "msm8x16-florida-snd-card",
		.dai_link	= msm8x16_florida_dai_links,
...
int populate_ext_snd_card_dt_data(struct platform_device *pdev)
{
	...
	if (IS_ERR(ext_cdc_pinctrl_info.tlmm_act)) {
		pr_err("%s: Unable to get pinctrl enable state handle\n",
								__func__);
		return -EINVAL;
	}
	...
	if (of_property_read_bool(pdev->dev.of_node,
		...
		memcpy(msm8x16_9302_dai_links + ARRAY_SIZE(msm8x16_dai),
			msm8x16_9302_dai, sizeof(msm8x16_9302_dai));
					...
					"qcom,ext-codec-clearwater")) {
		pr_debug("%s: CARD is clearwater\n", __func__);

		memcpy(msm8x16_clearwater_dai_links, msm8x16_dai,
				sizeof(msm8x16_dai));
		memcpy(msm8x16_clearwater_dai_links + ARRAY_SIZE(msm8x16_dai),
			msm8x16_clearwater_dai, sizeof(msm8x16_clearwater_dai));
	} else if (of_property_read_bool(pdev->dev.of_node,
					"qcom,ext-codec-florida")) {
		pr_debug("%s: CARD is florida\n", __func__);
...
static int msm8x16_asoc_machine_probe(struct platform_device *pdev)
{
...
#ifdef CONFIG_SND_SOC_MSM8X16_ARIZONA
		if (pdev->id == 3)
			pdata->arizona_type = WM1840; /* 8 */
		else if (pdev->id == 4)
			pdata->arizona_type = WM5110; /* 2 */
		else
			pdata->arizona_type = -1;
#endif
	} else {
	...
	atomic_set(&pdata->mclk_enabled, false);
#ifdef CONFIG_SND_SOC_MSM8X16_ARIZONA
	mutex_init(&pdata->ext_tlmm_mutex);
	atomic_set(&pdata->ext_tlmm_ref, 0);
#endif

#if defined(CONFIG_SND_SOC_MSM8X16_EXT_AMP) && !defined(CONFIG_TPA2026D2)
	/* initiallize ext amp gpio */
	pdata->ext_amp_en_gpio = of_get_named_gpio(pdev->dev.of_node,
					"qcom,ext-amp-ctrl-gpio", 0);
	if (pdata->ext_amp_en_gpio < 0) {
		dev_info(&pdev->dev, "no external amp control gpio\n");
	} else {
		gpio_request_one(pdata->ext_amp_en_gpio,
					GPIOF_OUT_INIT_LOW, "EXT_AMP_EN");
		if (ret)
			dev_err(&pdev->dev, "can't request %d, with err %d\n",
						pdata->ext_amp_en_gpio, ret);
		else
			dev_info(&pdev->dev,
					"ext-amp-ctrl-gpio has requested\n");
	}
#endif
...
#ifdef CONFIG_SND_SOC_MSM8X16_ARIZONA
	pdata->bbclk2 = clk_get(&pdev->dev, "bbclk2");
	if (IS_ERR(pdata->bbclk2)) {
		dev_err(&pdev->dev, "can't get 19.2MHz BBCLK2\n");
		goto err;
	} else {
		/* TODO : mclk1 control based on scenario */
		/* mclk1 is always on temporarily */
		clk_prepare(pdata->bbclk2);
		clk_enable(pdata->bbclk2);
		dev_info(&pdev->dev, "arizona mclk1 19.2MHz enabled\n");
	}
#endif
...
#endif /* CONFIG_SAMSUNG_JACK */
#ifdef CONFIG_SND_SOC_MSM8X16_ARIZONA
	if (init_voice_trigger_input_device() < 0)
		dev_err(&pdev->dev, "Can't init voice trigger device\n");
#endif
	return 0;
err:
...
#ifdef CONFIG_SND_SOC_MSM8X16_ARIZONA
	clk_disable(pdata->bbclk2);
	clk_put(pdata->bbclk2);
	deinit_voice_trigger_input_device();
#endif
