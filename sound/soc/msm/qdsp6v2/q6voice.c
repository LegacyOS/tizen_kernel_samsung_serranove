...
static int voice_create_mvm_cvs_session(struct voice_data *v)
{
	...
	if (!mvm_handle) {
		if (!is_voip_session(v->session_id)) {
			...
			pr_info("%s: mvm_session_cmd.mvm_session.name %s\n",
				__func__, mvm_session_cmd.mvm_session.name);
	...
	if (!cvs_handle) {
		if (!is_voip_session(v->session_id)) {
			...
			pr_info("%s: cvs_session_cmd.cvs_session.name: %s\n",
				__func__, cvs_session_cmd.cvs_session.name);
...
#ifdef CONFIG_SAMSUNG_AUDIO
		if ((loopback_mode == LOOPBACK_OFF) &&
					(loopback_prev_mode != LOOPBACK_OFF)) {
			ret = send_packet_loopback_cmd(v, LOOPBACK_OFF);
			if (ret < 0) {
	...
	if ((v->voc_state == VOC_INIT) ||
		...
		if (loopback_mode != LOOPBACK_OFF) {
			...
			if (ret < 0) {
				...
				pr_info("%s : loopback enable %d success\n",
...
static int voice_send_dha_data(struct voice_data *v)
{
	...
	vpcm_dha_param_send_cmd.hdr.src_port =
				voice_get_idx_for_session(v->session_id);
