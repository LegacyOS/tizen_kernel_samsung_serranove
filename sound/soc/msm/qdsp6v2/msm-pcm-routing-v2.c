			...
			struct snd_ctl_elem_value *ucontrol)
{
	...
	mutex_lock(&routing_lock);
	ac = q6asm_get_audio_client(fe_dai_map[MSM_FRONTEND_DAI_MULTIMEDIA3]
						[SESSION_TYPE_RX].strm_id);
	pr_info("%s: sa_ep  ret=%d score=%d\n", __func__,
					q6asm_get_sa_ep(ac), score);
...
static int lpa_mute_flag;
static int msm_sec_lpa_mute_get(struct snd_kcontrol *kcontrol,
			struct snd_ctl_elem_value *ucontrol)
{
	ucontrol->value.integer.value[0] = lpa_mute_flag;
	/*pr_info("%s: mute %d\n", __func__, lpa_mute_flag);*/
	return 0;
}

static int msm_sec_lpa_mute_put(struct snd_kcontrol *kcontrol,
			struct snd_ctl_elem_value *ucontrol)
{
	int ret = 0;
	struct audio_client *ac;
	...
	ret = q6asm_set_mute(ac, ucontrol->value.integer.value[0]);
	if (!ret) {
		lpa_mute_flag = ucontrol->value.integer.value[0];
		pr_info("%s: success value %d\n", __func__, lpa_mute_flag);
		msleep(50);
	} else {
		pr_err("%s: failed %d\n", __func__, ret);
	}
...
static const char *const ec_ref_rx[] = { "None", "SLIM_RX", "PRI_MI2S_RX",
	"PRI_MI2S_TX", "SEC_MI2S_TX", "TERT_MI2S_TX", "QUAT_MI2S_TX",
	"PROXY_RX", "QUAT_MI2S_RX"};
...
static const struct snd_kcontrol_new ec_ref_rx_mixer_controls[] = {
	SOC_ENUM_EXT("EC_REF_RX", msm_route_ec_ref_rx_enum[0],
					msm_routing_ec_ref_rx_get,
					msm_routing_ec_ref_rx_put_no_dapm),
};
...
static const struct snd_kcontrol_new ss_solution_mixer_controls[] = {
	...
	SOC_SINGLE_MULTI_EXT("LPA Mute", SND_SOC_NOPM, 0, 65535, 0, 1,
				msm_sec_lpa_mute_get, msm_sec_lpa_mute_put),
};
#endif
