...
#define LPA_LR_VOL_MAX_STEPS	0x2000
#else
		...
		unsigned int cmd, void *arg)
{
	...
	struct snd_pcm_runtime *runtime = substream->runtime;
	struct msm_audio *prtd = runtime->private_data;
#ifdef CONFIG_SND_SOC_MSM_QDSP6V2_COMPR
	uint64_t timestamp;
	uint64_t temp;
#endif
				  ...
				  struct snd_ctl_elem_value *ucontrol)
{
	...
	if (prtd) {
		if (lpa_start_vol) {
			lpa_start_vol = 0;
			rc = lpa_set_volume(prtd, 5);
			if (rc < 0)
				pr_info("%s: set workaround has failed %d\n",
					__func__, rc);
			else
				pr_info("%s: workaround for 0x%x\n",
					__func__, volume);
			msleep(20);
		}
