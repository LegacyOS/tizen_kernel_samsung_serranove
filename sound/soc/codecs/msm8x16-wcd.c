				     ...
				     struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		if (!msm8x16_wcd->spk_boost_set) {
			dev_info(w->codec->dev, "%s(PRE_PMU)(spk_boost_set %s\n",
						__func__, msm8x16_wcd->spk_boost_set ? "on":"off");
			snd_soc_update_bits(codec,
	...
	case SND_SOC_DAPM_POST_PMU:
		...
		if (msm8x16_wcd->spk_boost_set) {
			...
			dev_info(w->codec->dev, "%s(POST_PMU) spk_boost_set %s\n",
						__func__, msm8x16_wcd->spk_boost_set ? "on":"off");
		} else {
			snd_soc_update_bits(codec,
	...
	case SND_SOC_DAPM_PRE_PMD:
		...
		if (msm8x16_wcd->spk_boost_set) {
			...
			dev_info(w->codec->dev, "%s(PRE_PMD) spk_boost_set %s\n",
						__func__, msm8x16_wcd->spk_boost_set ? "on":"off");
		} else {
			snd_soc_update_bits(codec,
				MSM8X16_WCD_A_ANALOG_SPKR_DAC_CTL, 0x10, 0x00);
			    ...
			    int mclk_enable, bool dapm)
{
	...
	dev_info(codec->dev, "%s: mclk_enable = %u, dapm = %d\n",
...
static const struct msm8x16_wcd_reg_mask_val
	 ...
	 */
#if defined(CONFIG_SND_SOC_MSM_TIZEN_TUNE)
...
#if defined(CONFIG_SEC_Z3LTE_CIS_PROJECT) || (CONFIG_SEC_Z3LTE_CHN_PROJECT) || (CONFIG_SEC_SERRANOVE_PROJECT)
	{MSM8X16_WCD_A_ANALOG_MICB_1_INT_RBIAS, 0x40, 0x00},
#endif
};
