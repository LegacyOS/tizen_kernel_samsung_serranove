...
{
	return snd_soc_register_platform(dev, &clearwater_compr_platform);
}
EXPORT_SYMBOL_GPL(clearwater_asoc_platform_register);

void clearwater_asoc_platform_unregister(struct device *dev)
{
	snd_soc_unregister_platform(dev);
}
EXPORT_SYMBOL_GPL(clearwater_asoc_platform_unregister);
...
int clearwater_speaker_set_mute(struct snd_soc_codec *codec, int onoff)
{
	...
	ret = snd_soc_update_bits(codec, ARIZONA_DAC_DIGITAL_VOLUME_4L,
			ARIZONA_OUT4L_MUTE_MASK, value << ARIZONA_OUT4L_MUTE_SHIFT);
	if (ret < 0) {
		pr_info("%s: OUT4L set mute failed\n", __func__);
		return ret;
	}

	ret = snd_soc_update_bits(codec, ARIZONA_DAC_DIGITAL_VOLUME_4R,
			ARIZONA_OUT4R_MUTE_MASK, value << ARIZONA_OUT4R_MUTE_SHIFT);
	if (ret < 0) {
		pr_info("%s: OUT4R set mute failed\n", __func__);
		return ret;
	}

	return 0;
}
EXPORT_SYMBOL_GPL(clearwater_speaker_set_mute);
#endif
