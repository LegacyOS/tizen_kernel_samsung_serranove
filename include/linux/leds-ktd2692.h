...
#define LED_ERROR(x, ...) pr_err("%s : " x, __func__, ##__VA_ARGS__)
#define LED_INFO(x, ...) pr_info("%s : " x, __func__, ##__VA_ARGS__)
#define LED_CHECK_ERR_GOTO(x, out, fmt, ...) do { \
	if (unlikely((x) < 0)) { \
...
#define T_L_HB		(4)	/* us */
#define T_SOD		(10+5)	/* us */
#define T_EOD_L		4	/* us */
#define T_EOD_H		(350+10)	/* us */
