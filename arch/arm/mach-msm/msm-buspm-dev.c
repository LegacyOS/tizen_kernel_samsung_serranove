		...
		case MSM_BUSPM_BUS_MIN_CLR:
			{
				...
				if (bs.op == MSM_BUSPM_BUS_MIN_SET) {
					...
					pr_err("ALRAN: %d NOC %u Khz MIN\n", bs.nocid, bs.min);
				}
				else {
					clk_disable_unprepare(clk);
					pr_err("ALRAN: %d NOC MIN CLR\n", bs.nocid);
				}
			}
			break;
		default:
