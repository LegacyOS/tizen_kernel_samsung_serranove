...
void ist30xxc_tsp_charger_infom(int cable_type, int attach)
{
	if (!ist30xxc_charger_callbacks)
		return;

	if (!ist30xxc_charger_callbacks->inform_charger)
		return;
	...
	case CABLE_TYPE_NONE:
		ist30xxc_charger_callbacks->inform_charger(ist30xxc_charger_callbacks, attach);
		break;
	...
	case CABLE_TYPE_OTG:
	default:
		ist30xxc_charger_callbacks->inform_charger(ist30xxc_charger_callbacks, false);
		break;
	}
...
extern void tsp_charger_infom(int en);
#endif
...
void sm5502_muic_init_cb(void)
{
	...
	if (ret < 0) {
        ...
        if (ret < 0) {
                pr_err("Failed to register USB cable switch. %d\n", ret);
        }
}
...
#ifdef CONFIG_TOUCHSCREEN_IMAGIS_IST30XXC
	ist30xxc_tsp_charger_infom(cable_type, attached);
#endif

#if defined(CONFIG_TOUCHSCREEN_MMS252)
        tsp_charger_infom(attached);
#endif
...
#endif
	.muic_init_cb = sm5502_muic_init_cb,
...
static struct switch_dev switch_jig = {
	.name = "jig_cable",	/* /sys/class/switch/jig_cable/state */
};
static struct switch_dev switch_usb = {
		.name = "usb_cable",
		};
...
#endif
		break;
	case CABLE_TYPE_NON_STND_AC:
#if defined(DEBUG_STATUS)
			if (attached)
			{
				   status_count = status_count+1;
				   pr_err("%s Non Standard Charger status attached (%d) \n",__func__, status_count);
			} else {
				   status_count = status_count-1;
				   pr_err("%s Non Standard charger status detached (%d) \n", __func__,status_count);
			}
#endif
		break;
	case CABLE_TYPE_UARTOFF:
